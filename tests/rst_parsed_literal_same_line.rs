...
fn header_footer_replace_epigraph_unchanged() {
    let header = concat!(".. header:: fig. 1 is here. After.\n", "After. Next.\n",);
    let header_out = format_text(header, &rst_cfg()).unwrap();
    assert_eq!(
        header_out,
        concat!(
        ...
        "header leftover from #422 must stay, got:\n{header_out}"
    );

    let footer = concat!(".. footer:: fig. 1 is here. After.\n", "After. Next.\n",);
    let footer_out = format_text(footer, &rst_cfg()).unwrap();
    assert_eq!(
        footer_out,
        concat!(
        ...
        "footer leftover from #422 must stay, got:\n{footer_out}"
    );
