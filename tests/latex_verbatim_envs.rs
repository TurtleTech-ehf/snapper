...
//! GitHub #400: tcolorbox \\tcbinputlisting.
//! GitHub #399: fancyvrb.sty \\VerbatimInput / \\BVerbatimInput /
//! \\LVerbatimInput is one leftover command; following flush prose
...
//! GitHub #441: pythontex.sty leftover inline \\py / \\pyc / \\pys /
...
//! does not join.
//! GitHub #462: catchfile.sty leftover \\CatchFileDef / \\CatchFileEdef
//! stay one atomic command; following flush prose does not join.
//! GitHub #468: fvextra leftover buffer \\VerbatimInsertBuffer /
//! \\VerbatimClearBuffer / \\InsertBuffer / \\IterateBuffer stay one
//! atomic Structure command; following flush prose does not join.
//! GitHub #473: fancyvrb leftover \\DefineShortVerb / \\UndefineShortVerb
//! stay one atomic Structure command; following flush prose does not join.
...
/// stays one atomic command. Following flush `After.` does not join
/// the command line. `After.` / `Next.` still split. B/L twins and
...
/// Ticket fixture (GitHub #471): moreverb.sty `\verbatimwrite{file}`
/// / `\verbatimwrite*` stay one atomic command. Following flush
/// `After.` does not join the command line. `After.` / `Next.` still
/// split. verbatimtabinput / listinginput unchanged. extras skip so a
/// configured extra does not re-tokenize the no-brace form as Delim.
#[test]
fn verbatimwrite_fixture_does_not_join_following_prose() {
...
fn sagetex_inline_cmds_do_not_join_following_prose() {
    ...
    let extras_str = format_text("Before. Next.\n\\sagestr After. Next.\n", &extras_cfg).unwrap();
    assert!(
...
/// `After.` / `Next.` still split. Longer name first so it is not
...
fn pythontexcustomc_fixture_does_not_join_following_prose() {
    ...
    assert!(
        env_out.contains("\\begin{pythontexcustomcode}{py}"),
        "pythontexcustomcode env must stay unchanged, got:\n{env_out}"
    );
    assert!(
        env_out.contains("After.\nNext."),
        "prose after pythontexcustomcode env must still split, got:\n{env_out}"
    );
...
/// `\pyth{code}` / `\pyth|code|` stays one atomic command. Following
/// flush `After.` does not join the command line. `After.` / `Next.`
/// still split. Longer name first so it is not `\py` + leftover.
/// `\inputpython` / `\inputpythonfile` / `\pythontexcustomc` / `\py`
/// unchanged. extras skip so a configured extra does not re-tokenize
/// the no-brace form as Delim.
#[test]
fn pyth_fixture_does_not_join_following_prose() {
...
#[test]
fn inputpython_fixture_does_not_join_following_prose() {
    let input = concat!(
        "Before. Next.\n",
        "\\inputpython{foo.py}{1}{20}\n",
        "After. Next.\n",
...
/// `\sympy` / `\pylab` and twins stay one Structure span. Following
/// flush `After.` does not join. `After.` / `Next.` still split.
/// `\inputpy` / `\inputpygments` / `\pygment` unchanged.
#[test]
fn pytx_inline_fixture_does_not_join_following_prose() {
    for name in ["py", "pyc", "pys", "pyb", "pyv", "pycon", "sympy", "pylab"] {
...
/// `Next.` still split. `\CatchFileBetweenTags` stays atomic. extras
/// skip so a configured extra does not re-tokenize the no-brace form
/// as Delim.
#[test]
fn catchfile_leftover_fixture_does_not_join_following_prose() {
    for cmd in [
...
/// Ticket fixture (GitHub #468): fvextra leftover buffer cmds stay one
/// atomic command. Following flush `After.` does not join. `After.` /
/// `Next.` still split. `\UseVerb{foo}` stays atomic. extras skip so a
/// configured extra does not re-tokenize the no-brace form as Delim.
#[test]
fn fvextra_buffer_leftover_cmds_fixture_does_not_join_following_prose() {
    for cmd in [
...
/// `\DefineShortVerb{\|}` / `\UndefineShortVerb{\|}` stay one atomic
/// command. Following flush `After.` does not join. `After.` / `Next.`
/// still split. `\UseVerb{foo}` stays atomic. extras skip so a
/// configured extra does not re-tokenize the no-brace form as Delim.
#[test]
fn fancyvrb_shortverb_leftover_cmds_fixture_does_not_join_following_prose() {
...
/// extras skip so a configured extra does not re-tokenize the no-brace
/// form as Delim.
#[test]
