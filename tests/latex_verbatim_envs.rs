...
//! are the same verbatim@start raw class.
...
/// `listing*` / `listingcont*` bodies stay Code; `listing` / `listing*`
/// keep `{1}` on begin; following prose still splits.
#[test]
fn moreverb_listing_envs_fixture_is_code_and_does_not_reflow() {
    let cases = [
        ("listing", Some("{1}")),
        ("listingcont", None),
        ("listing*", Some("{1}")),
