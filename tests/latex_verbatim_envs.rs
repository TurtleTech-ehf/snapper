//! snapper-3tj3 / GitHub #98: Overleaf verbatimEnvNames stay Code.
...
#[test]
fn ticket_verbatim_fixture_is_code_not_prose() {
    ...
    assert_eq!(
        out, input,
        "ticket Verbatim fixture must be identity, got:\n{out}"
    );
    assert!(
        !out.contains("First line.\nSecond line."),
        "Verbatim body must not reflow as prose, got:\n{out}"
    );
...
fn boxedverbatim_tcblisting_codeexample_do_not_reflow() {
    for name in ["boxedverbatim", "tcblisting", "codeexample"] {
        ...
        let out = format_text(&input, &latex_cfg()).unwrap();
        assert_eq!(out, input, "{name} must be identity, got:\n{out}");
        assert!(
            !out.contains("First line.\nSecond line."),
