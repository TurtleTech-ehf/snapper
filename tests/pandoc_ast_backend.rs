...
#[test]
fn format_text_use_pandoc_rst_comment_is_kept() {
    if !require_writer() {
        return;
    }
...
#[test]
fn format_text_use_pandoc_snapper_off_is_kept() {
    if !require_writer() {
        return;
    }
    ...
    assert!(
        ...
        "off-region body must survive un-reflowed:\n{out}"
    );
    assert_has_sentence_break(&out, "Hello world.", "Second sentence");
    assert_has_sentence_break(&out, "After.", "Two");
}

#[test]
fn format_text_use_pandoc_rst_snapper_off_is_kept() {
    if !require_writer() {
        return;
    }
    ...
    assert!(
        out.contains("snapper:off") && out.contains("snapper:on"),
        "RST pragma markers must survive:\n{out}"
    );
    assert!(
        out.contains("Keep this. Exactly here."),
...
fn snapper_cli_use_pandoc_rst_comment_exits_0() {
    ...
    assert!(
        ...
        "CLI write must keep RST comment body:\n{stdout}"
    );
    assert!(
        stdout.contains(".. This is a recognized comment."),
        "CLI write must keep recognized RST comment:\n{stdout}"
    );
}
...
fn snapper_cli_use_pandoc_snapper_off_exits_0() {
    ...
    assert!(
        ...
        "CLI write must keep snapper:off and off-region body:\n{stdout}"
    );
}
