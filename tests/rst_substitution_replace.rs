...
#[test]
fn same_line_replace_does_not_swallow_flush_paragraph() {
    let input = concat!(
        ".. |v| replace:: fig. 1 is here. After.\n",
    ...
    assert_eq!(
        ...
        "After markup must stay flush after same-line replace, got:\n{out}"
    );
    assert!(
        !out.contains("\n                 After markup."),
        "After markup must not inherit the replace hang, got:\n{out}"
    );
