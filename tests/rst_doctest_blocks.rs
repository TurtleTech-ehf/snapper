...
/// snapper-ijxt: prompt-only `>>>` is Body.doctest, not a `>` underline.
...
#[test]
fn prompt_only_doctest_is_structure_not_prose() {
    ...
    assert!(
        ...
        "prompt-only >>> must be one Structure region, got {regions:?}"
    );
    assert!(
        !regions.iter().any(|r| matches!(
            r,
...
fn prompt_only_doctest_is_identity_under_format() {
    ...
    assert_eq!(
        out, input,
        "prompt-only >>> must stay unjoined, got:\n{out}"
    );
    assert!(
    ...
    assert_eq!(
        format_text(&out, &rst_cfg()).unwrap(),
        out,
        "hung prompt-only doctest must be identity, got:\n{out}"
    );
}
