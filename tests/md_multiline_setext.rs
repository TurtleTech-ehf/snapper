...
/// GitHub #261 / CommonMark 5.2: setext underline must sit at the list
/// hang. Hang of `1. ` is 3; two spaces is below hang.
#[test]
fn ordered_list_underline_below_hang_is_not_setext() {
    ...
    assert!(
        out.contains("list item.\n   Still item."),
        "Foo stays Prose and still splits at hang 3, got:\n{out}"
    );
    assert!(
...
/// Hang of `- ` is 2, so two-space `=======` stays a heading.
#[test]
...
/// Quoted list hang of `-` is 2. One space after the quote marker is
/// below hang; two spaces sit at hang and stay a heading.
#[test]
fn quoted_list_underline_below_hang_is_not_setext() {
    ...
    assert!(
        ...
        "quoted > - Foo / >   ======= must stay a heading, got: {at_hang_regions:?}"
    );
}
