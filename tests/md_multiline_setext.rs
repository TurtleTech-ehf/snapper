...
/// GitHub #258: a later same-depth list opener ends the quoted item.
/// `2.` after `> 1. Foo` is a sibling, not title text. Hung underline
/// promotes only Bar.
#[test]
fn quoted_sibling_start_not_one_is_not_prior_list_heading() {
    for opener in ["> 1. Foo", "> - Foo"] {
        ...
        assert!(
            ...
            "{opener}: Bar heading text must survive, got:\n{out}"
        );
        assert!(
