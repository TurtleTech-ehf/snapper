//! snapper-awlr / GitHub #430: leftover body.py `line-block`
...
#[test]
fn indented_line_block_directive_body_still_hangs() {
    let input = concat!(
        ".. line-block::\n",
        "\n",
        "   fig. 1 is here. After.\n",
    ...
    assert_eq!(
        ...
        "indented line-block body must hang and split, got:\n{out}"
    );
    assert_eq!(format_text(&out, &rst_cfg()).unwrap(), out);
}

#[test]
fn pipe_line_block_still_ends_at_flush() {
    let input = concat!("| First line. Second line.\n", "After. Next.\n",);
    let out = format_text(input, &rst_cfg()).unwrap();
    assert_eq!(
        out,
        concat!("| First line.\n", "  Second line.\n", "After.\n", "Next.\n",),
        "| line-block must still end at flush prose, got:\n{out}"
    );
    assert_eq!(format_text(&out, &rst_cfg()).unwrap(), out);
}
