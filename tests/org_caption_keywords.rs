...
//!
//! org-element-parsed-keywords is CAPTION only. `#+CAPTION:` / dual
//! `#+CAPTION[short]:` opener is Structure; the value hangs and splits.
...
/// Ticket fixture (Format::Org / GitHub #204).
...
fn caption_opener_is_structure_value_is_hung_prose() {
    ...
    assert!(
        ...
        "link line must be Structure, got {regions:?}"
    );
}
...
fn caption_fixture_value_splits_and_hangs() {
    ...
    assert_eq!(
        ...
        "CAPTION opener stays; value hangs and splits; following prose reflows, got:\n{out}"
    );
    assert!(
