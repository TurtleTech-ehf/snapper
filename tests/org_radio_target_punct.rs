//! GitHub #211 / snapper-gz10: org-element-radio-target-parser /
//! org-radio-target-regexp `<<<contents>>>` and org-target-regexp
//! `<<contents>>` must stay one inline token. Interior punctuation
//! is not a sentence boundary. The use stays Prose.
...
fn ticket_radio_use_stays_prose() {
    ...
    assert!(
        regions.iter().any(|r| matches!(
            r,
            Region::Prose(s) if s.contains("<<<the Fourier. transform>>>")
        )),
...
#[test]
fn angle_target_stays_one_span_and_splits_next() {
