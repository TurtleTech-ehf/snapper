...
#[test]
fn spaced_shortcut_references_stay_two_spans() {
    // CM 0.31.2 §6.3 examples 542-543 / snapper-9xjl.
    let input = "See [foo] [bar] now. Next sentence.\n";
    let out = format_text(input, &md_cfg()).unwrap();
    assert!(
        out.contains("[foo] [bar]"),
        "spaced shortcuts must not be rewritten, got:\n{out}"
    );
    assert!(
        out.contains("now.\nNext sentence."),
        "following sentence must still split, got:\n{out}"
