...
* `--use-pandoc` — Use pandoc as parser backend (universal format support). RST comments and `snapper:off` / `snapper:on` stay in the written output (not deleted). Parse may use in-process FFI; the writer still needs `pandoc` on PATH (`libsnapper_pandoc` is reader-only) unless that library exports a writer. Without FFI, both parse and write use the `pandoc` CLI (explicit error if missing)
