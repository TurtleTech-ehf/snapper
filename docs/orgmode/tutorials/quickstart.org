...
Published =snapper= / =snapper-fmt= binaries include the MCP server.
Configure an MCP client to run the stdio command:

#+begin_src bash
snapper mcp
#+end_src

Agents should call this server or the =snapper= CLI instead of applying [[https://sembr.org/][sembr.org]] / [[https://github.com/sembr/skills][sembr/skills]] wrapping by hand.
...
Use the source in =editors/obsidian= for development builds.
...
Use the source and sideloading instructions in =editors/word=.
