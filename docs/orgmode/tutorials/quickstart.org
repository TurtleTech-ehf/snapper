...
Each sentence occupies its own line. Structural elements like code blocks,
...
Use =--check= mode to verify formatting without modifying files. Exits with
code 1 if any file would change:
