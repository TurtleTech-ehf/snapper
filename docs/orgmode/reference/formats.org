...
- Setext headings (one or more title lines plus ====== or ------ underline, including list/quote items, hard-break title lines, a hard-break plus 4-space continuation, and a 4-space =<!--= continuation). A lazy unquoted underline after a quote is not a heading (CommonMark 4.3 ex. 93). A quote or list opener as the last title line interrupts the prior paragraph; a deeper quote prefix or a list opener after an open quote is the same interrupt; only the new container is the heading. A quoted underline matches only the heading container's marker depth (shallower is lazy; deeper opens a nested quote). An empty quote line (=>= / => = / =>   =) closes the quote paragraph so a later same-depth setext is a new heading
- List item markers (=-=, =*=, =+=, =1.=); continuation sentences hang at the marker width
