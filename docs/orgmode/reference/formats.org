...
:END:
- Non-source =#+BEGIN_*= ... =#+END_*= blocks (example, quote, etc.)
...
- =\end{document}=
- Sectioning command wrappers (=\section{=, =\subsection{=, …); the brace argument is prose
...
- Sectioning command titles (argument text inside =\section{...}= and friends)
