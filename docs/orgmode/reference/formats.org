...
- Inline source: =src_lang{...}= / =src_lang[headers]{...}= (org-element-inline-src-block-parser; =\<src_= is word-start; =foo_src_= is a subscript, so leftover braces stay prose)
- Inline babel call: =call_name(...)= / =call_name[inside](...)[end]= (org-element-inline-babel-call-parser; same =\<call_= word-start; =foo_call_= is a subscript)
