...
- Inline =\verb= / =\lstinline= / =\spverb= / =\mintinline= / =\mint= / fancyvrb =\Verb= / =\Verb*= / =\SaveVerb= / piton.sty =\piton= (and extra =[latex].verbatim_commands=) stay atomic; inner =.!?%= do not split or comment. =\piton|...|= is verb-like; =\piton{...}= stays one token via the generic command argument
