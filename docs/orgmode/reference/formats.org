...
- Inline source: =src_lang{...}= / =src_lang[headers]{...}= (org-element-inline-src-block-parser)
- Inline babel call: =call_name(...)= / =call_name[inside](...)[end]= (org-element-inline-babel-call-parser)
...
- Setext headings (one or more title lines plus ====== or ------ underline, including list items, fully marked quote items, hard-break title lines, and a 4-space continuation; a lazy unquoted underline after a quote is not a heading, CommonMark 4.3 ex. 93)
- List item markers (=-=, =*=, =+=, =1.=); continuation sentences hang at the marker width
