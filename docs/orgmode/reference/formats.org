...
- Setext headings (one or more title lines plus ====== or ------ underline, including list/quote items, hard-break title lines, a hard-break plus 4-space continuation, and a 4-space =<!--= continuation). A lazy unquoted underline after a quote is not a heading (CommonMark 4.3 ex. 93). A shallower or deeper quote prefix on the underline is the same reject at nested depth. An empty quote line closes the paragraph. A quote or list opener as the last title line interrupts the prior paragraph; a deeper quote prefix or a list opener after an open quote is the same interrupt, including when the opener is mid-title; only the new container is the heading. A list item plus a column-0 underline (after a shared quote prefix) is outside the item. A =>= marker may be indented 0-3 spaces; four spaces or a tab is not a closer
- List item markers (=-=, =*=, =+=, =1.=); continuation sentences hang at the marker width
