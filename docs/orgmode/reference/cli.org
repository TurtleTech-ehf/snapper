...
snapper sdiff [--no-color] [--word-level] [-f FORMAT] <OLD> <NEW>
snapper git-diff [--no-color] [--word-level] [-f FORMAT] [REF] [FILE...]
snapper wdiff [--output-format FORMAT] [-f FORMAT] <OLD> <NEW>
...
*** =snapper wdiff <OLD> <NEW>=

Word-level diff between two files (latexdiff-style).
Compares sentences, then highlights which words changed within each modified sentence.

Use =--output-format= to select markup style: =terminal= (default), =plaintext=, =latex=, =markdown=, =org=, =typst=.

LaTeX output generates =\DIFadd{}= / =\DIFdel{}= compatible with latexdiff's DIF style.
