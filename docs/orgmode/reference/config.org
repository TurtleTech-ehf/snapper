...
Built-in: =minted=, =lstlisting= / =lstlisting*=, =verbatim=, =comment=, Overleaf =Verbatim= / =boxedverbatim= / =tcblisting= / =tcblisting*= / =codeexample=, moreverb =verbatimtab=, fancyvrb =BVerbatim= / =BVerbatim*= / =LVerbatim= / =LVerbatim*= / =Verbatim*= / =SaveVerbatim= / =VerbatimOut=, fvextra =VerbatimWrite=, standard =alltt=, spverbatim.sty =spverbatim=, and pythontex.sty =pycode= / =pyblock= / =pyverbatim= / =pyconsole=.
...
Built-in: the =NON_PROSE_ENVS= list (equation, align, tabular, tikzpicture, and starred variants, plus the built-in code envs). Figure/table chrome stays Structure; the =\caption= long argument is Prose.
...
Built-in names are =verb=, =lstinline=, =spverb=, fancyvrb =Verb= / =Verb*=, =mintinline=, and =mint=; =lstinline= still accepts optional =[...]= and ={...}=. =mintinline= / =mint= take optional =[...]=, a ={lang}= argument, then a delimiter or ={...}= body.
Adding an unlisted name keeps that command's delimited body atomic and treats an inner =%= as content, not a comment.
