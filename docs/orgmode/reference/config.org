...
Built-in names are =verb=, =lstinline=, =lstinputlisting=, =spverb=, =mintinline=, =inputminted=, =mint=, fancyvrb =Verb= / =Verb*= / =SaveVerb= / =VerbatimInput= / =BVerbatimInput= / =LVerbatimInput=, piton.sty =\piton= / =\PitonInputFile= / =\PitonInputFileT= / =\PitonInputFileF= / =\PitonInputFileTF=, tools/verbatim.sty =\verbatiminput=, tcolorbox =\tcbinputlisting=, pythontex.sty =\inputpy= / =\inputpycon= / leftover inline =\py= / =\pyc= / =\pys= / =\pyb= / =\pyv= / =\pycon= and twins / =\sympy= / =\pylab= and twins / =\inputpygments= / =\pygment=, pythonhighlight.sty =\inputpython= / =\inputpythonfile=, catchfilebetweentags.sty =\CatchFileBetweenTags= / =\CatchFileBetweenDelims= / =\ExecuteMetaData=, moreverb =\listinginput=, and sagetex =\sageinput=; =lstinline= still accepts optional =[...]= and ={...}=.
