...
Built-in names are =verb=, =lstinline=, =lstinputlisting=, =spverb=, =mintinline=, =inputminted=, =mint=, fancyvrb =Verb= / =Verb*= / =SaveVerb=, piton.sty =\piton=, tools/verbatim.sty =\verbatiminput=, and tcolorbox =\tcbinputlisting=; =lstinline= still accepts optional =[...]= and ={...}=.
