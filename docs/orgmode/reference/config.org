...
Built-in names are =verb=, =lstinline=, =lstinputlisting=, =spverb=, =mintinline=, =inputminted=, =mint=, fancyvrb =Verb= / =Verb*= / =SaveVerb= / =VerbatimInput= / =BVerbatimInput= / =LVerbatimInput=, piton.sty =\piton= / =\PitonInputFile=, tools/verbatim.sty =\verbatiminput=, tcolorbox =\tcbinputlisting=, pythontex.sty =\inputpy= / =\inputpycon=, and moreverb =\listinginput=; =lstinline= still accepts optional =[...]= and ={...}=.
