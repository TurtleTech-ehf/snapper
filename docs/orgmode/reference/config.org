...
Built-in names are =verb=, =lstinline=, =lstinputlisting=, =spverb=, =mintinline=, =inputminted=, =mint=, fancyvrb =Verb= / =Verb*= / =SaveVerb= / =VerbatimInput=, piton.sty =\piton=, tools/verbatim.sty =\verbatiminput=, tcolorbox =\tcbinputlisting=, and pythontex.sty =\inputpy= / =\inputpycon=.
