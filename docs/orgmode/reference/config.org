...
Built-in names are =verb=, =lstinline=, =spverb=, =mintinline=, and =mint=; =lstinline= still accepts optional =[...]= and ={...}=; =mintinline= / =mint= take a required ={lang}= then a delimiter or ={...}=.
