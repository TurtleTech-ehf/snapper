...
Built-in names are =verb=, =lstinline=, =spverb=, =mintinline=, =mint=, =inputminted=, fancyvrb =Verb= / =Verb*= / =SaveVerb=, and piton.sty =\piton=; =lstinline= still accepts optional =[...]= and ={...}=.
=mintinline= / =mint= take optional =[...]=, a ={lang}= argument, then a delimiter or ={...}= body.
=\inputminted= takes optional =[...]=, a ={lang}= argument, then a ={filename}=; following flush prose stays on its own line.
