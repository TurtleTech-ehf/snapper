...
#+begin_src yaml
- uses: TurtleTech-ehf/snapper@v0.11.2
  with:
    files: '**/*.org **/*.tex **/*.md'
#+end_src
...
This runs =snapper --in-place= on changed files matching =.org=, =.tex=/=.latex=, =.md=/=.markdown=, =.rst=, and =.txt= (extension match; pre-commit identify has no =org= type).
Requires a Rust toolchain to build the hook from this repo.
