...
=snapper init= writes this filter into the local git config as
=snapper-fmt --native --stdin-filepath %f= (extension auto-detect; =snapper-fmt=
avoids openSUSE's Btrfs =snapper=) plus a working pre-commit hook.
...
Do not attach this =--format org= filter to =*.tex= or =*.md=.
Those extensions need their own clean command (see per-format filters below).
...
#+begin_src bash
git config filter.snapper-org.clean "snapper --native --format org"
git config filter.snapper-org.smudge cat

git config filter.snapper-tex.clean "snapper --native --format latex"
