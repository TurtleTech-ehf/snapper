...
1. *Auto* — =--pandoc-backend auto= (default): prefer in-process FFI when =libsnapper_pandoc= loads, else CLI. This is the successor path for multi-format work (amortizes RTS; avoids process-per-file spawn).
...
**FFI backend (co-linked).** Feature =pandoc-colink= links =libsnapper_pandoc= at build time and embeds rpath for the foreign library plus GHC/pandoc package dirs (not host glibc). Happy path needs no =SNAPPER_PANDOC_LIB= discovery:
...
See =native/snapper-pandoc/README.md= for SONAME layout (=libsnapper_pandoc.so.0=).
