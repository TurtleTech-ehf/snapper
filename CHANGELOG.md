...
- (**markdown**) a setext underline inside a quote must match the heading container's `>` depth (shallower is lazy, deeper is a nested quote); an empty quote line (`>` / `> ` / `>   `) closes the paragraph so a later same-depth setext does not promote Foo (#208)
