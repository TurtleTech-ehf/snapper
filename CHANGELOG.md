...
- (**markdown**) setext titles promote the open CommonMark paragraph: list/quote items, hard-break lines, and a 4-space `<!--` continuation stay title text; a hard-break plus 4-space line stays title (not indented code); a lazy unquoted `=======` after a quote is still the quote paragraph (CommonMark 4.3 ex. 93); a new quote or list opener interrupts so the prior paragraph stays prose (#208)
