...
- (**org**) inline `src_lang{...}` and `call_name(...)` stay one token so interior punctuation is not a sentence or wrap boundary; `foo_src_` / `foo_call_` are subscripts, so leftover braces stay prose (#214)
