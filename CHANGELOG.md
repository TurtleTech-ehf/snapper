...
- (**reflow**) `--max-width` keeps links, images, inline code, autolinks, `$math$`, and Org `[[...]]` on one line (overlong token may sit alone); wrap-created Markdown lines that would parse as a new block (`-` `*` `+` `>` `#` `1.`) are backslash-escaped, other formats skip the cut; no NBSP; first line of a list item stays a list; second pass does not accumulate backslashes
