...
- (**markdown**) a setext underline inside a quote matches only the heading container's `>` depth (shallower is lazy, deeper is a nested quote); an empty quote line (`>` / `> ` / `>   `) closes the paragraph so a later same-depth setext is a new heading (#208)
