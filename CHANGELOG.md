...
- (**markdown**) a setext underline inside a quote must match the heading container's `>` depth (shallower is lazy, deeper is a nested quote); an empty quote line (`>` / `> ` / `>   `) closes the paragraph so a later same-depth setext does not promote Foo; a 4-space or tab `>` is not a matching-depth closer; a quoted list plus a quote-relative column-0 underline is outside the item; a nested `>>` or `> -` opener mid-title closes the outer paragraph (#208)
- (**markdown**) a quote or list opener as the last setext title line interrupts the prior paragraph; a deeper quote prefix or a list after an open quote is the same interrupt; only the new container is the heading (#208)
