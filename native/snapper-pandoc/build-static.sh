...
# Windows: MinGW ld rejects nested ar members from ghc -staticlib
# ("… is not an object"). Flatten with repack-coff.py (parses ar once —
# no per-member ar x; strips CR from Windows ar t names; unique names so
# Types.o collisions cannot drop packages).
case "$uname_s" in
  MINGW*|MSYS*|CYGWIN*)
    AR_BIN=""
    if [[ -n "${SNAPPER_MINGW_BIN:-}" ]]; then
      for a in ar x86_64-w64-mingw32-ar llvm-ar; do
        if [[ -x "${SNAPPER_MINGW_BIN}/${a}.exe" || -x "${SNAPPER_MINGW_BIN}/${a}" ]]; then
          AR_BIN="${SNAPPER_MINGW_BIN}/${a}"
          break
        fi
      done
    fi
    ...
    if [[ -z "$PY_BIN" ]]; then
      echo "build-static: ERROR python3 required for Windows COFF repack" >&2
      exit 1
    fi
    ARCHIVE_ABS="$(cd "$(dirname "$BUILD_DIR/libsnapper_pandoc.a")" && pwd)/libsnapper_pandoc.a"
    OUT_ABS="$(cd "$OUT_DIR" && pwd)/libsnapper_pandoc.a"
    echo "build-static: windows repack via repack-coff.py AR=$AR_BIN PY=$PY_BIN"
    "$PY_BIN" "$ROOT/repack-coff.py" "$ARCHIVE_ABS" "$OUT_ABS" --ar "$AR_BIN"
    ;;
esac
