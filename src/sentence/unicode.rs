...
pub fn protect_inline_tokens_with(
    ...
    // After `\verb|` / `\Verb|` so the delimiter is gone. A leftover
    // letter prefix before `|` is not a substitution_ref (Docutils
    // start-string; GitHub #233).
...
/// so inner `.!?%` cannot split or comment.
...
/// Byte end of a `\verb` / `\lstinline` / `\spverb` / `\Verb` / extra-name span starting at `at`.
///
/// `\verb` / `\verb*` / `\spverb` / `\spverb*` / `\Verb` / `\Verb*`: next
/// character is the delimiter; content runs to the same character.
/// `\lstinline` / `\lstinline*` may take optional `[...]` before a
/// delimiter or a `{...}` brace body. Extra names are tokenized like
...
/// Start-string must be at BOS or after 7-bit non-alphanumeric, so a
/// letter prefix (`\cmd|a.b|`) is not a substitution (GitHub #233).
