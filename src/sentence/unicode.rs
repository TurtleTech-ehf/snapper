...
/// `\InsertBuffer[...]` / `\IterateBuffer[...]{cmd}` /
/// `\DefineShortVerb[...]{\|}` / `\UndefineShortVerb{\|}` so inner `.!?%` cannot
...
/// `\UndefineShortVerb` / extra-name span
/// starting at `at`.
///
...
/// leftover; GitHub #462) take three required braces
/// `{macro}{file}{setup}`. Longer names first so this is not
/// CatchFile + leftover. No `*` form. `\CatchFileBetweenTags` /
/// `\CatchFileBetweenDelims` / `\ExecuteMetaData` stay their own
...
/// `\setpythontexcustomcode` is preamble-only. `\pyth`
/// (pythonhighlight.sty leftover; GitHub #450) takes a delimiter or
/// `{code}` body like `\lstinline`. Longer name first so it is not
/// `\py` + leftover. An ASCII-letter next token is not a delimiter,
/// so `\pyth After.` is not a span. There is no `*` form
/// (`\newcommand*`). `\inputpython` / `\inputpythonfile` / `\py` stay
...
/// No brace is not a span for `\IterateBuffer`. No `*` form.
/// fancyvrb leftover `\DefineShortVerb` / `\UndefineShortVerb`
/// (GitHub #473): optional `[...]` then required `{char}`. Longer
/// name first so `\UndefineShortVerb` is not leftover of a shorter
/// name. No brace is not a span. No `*` form. `\UseVerb` / `\Verb`
/// stay their own leftovers.
/// Extra names are tokenized like
...
pub(crate) fn latex_verb_span_end_with(
    ...
    let (mut i, kind) = if let Some(stripped) = tail.strip_prefix("mintinline") {
        ...
        // sagetex leftover inline (GitHub #446). Longer names first so
        ...
        // pythonhighlight.sty leftover (GitHub #450). Longer name first
        // so `\pyth` is not `\py` + leftover. Delimiter or `{code}`
        // body like `\lstinline`. No `*` form.
        ...
        // `{macro}{file}{setup}`. Longer names first so this is not
        // CatchFile + leftover. No `*` form. `\CatchFileBetweenTags`
        // / `\CatchFileBetweenDelims` / `\ExecuteMetaData` stay their
        // own leftovers.
        (after_bs + name.len(), VerbKind::CatchFileDef)
    ...
    } else if let Some(name) = fvextra_buffer_leftover_cs_name(tail) {
        // fvextra leftover buffer cmds (GitHub #468). Longer names
        // first so `\VerbatimInsertBuffer` is not `\Verb` + leftover.
        // `\IterateBuffer` requires `{cmd}`; the others take optional
        // `[...]` only. No `*` form. `\UseVerb` / `\VerbatimInput`
        // stay their own leftovers.
        let kind = if name == "IterateBuffer" {
            VerbKind::FvextraIterateBuffer
        } else {
        ...
        (after_bs + name.len(), VerbKind::Lstinputlisting)
    } else if let Some(name) = fancyvrb_shortverb_cs_name(tail) {
        // fancyvrb leftover `\DefineShortVerb` / `\UndefineShortVerb`
        // (GitHub #473). Optional `[...]` then required `{char}`.
        // Longer name first so `\UndefineShortVerb` is not leftover
        // of a shorter name. No `*` form. `\UseVerb` / `\Verb` stay
        // their own leftovers.
        (after_bs + name.len(), VerbKind::Lstinputlisting)
    ...
    // for VerbatimInsertBuffer / InsertBuffer / IterateBuffer.
    // `\VerbatimClearBuffer` takes no args. `\IterateBuffer` then
    // requires `{cmd}`; no brace is not a span. The others end after
    // the optional.
    if matches!(
        kind,
        VerbKind::FvextraBuffer | VerbKind::FvextraIterateBuffer
    ) {
        let after_name = i;
        ...
        } else if kind == VerbKind::FvextraBuffer {
            // No optional: span is the name only. Do not swallow
            // following flush prose as if it were a delimiter.
            return Some(after_name);
        }
        if kind == VerbKind::FvextraIterateBuffer {
            return skip_required_brace_groups(text, i, 1);
        }
        return Some(i);
    }
    ...
    // `[ltx opts][fmt]` then required `{graphics}`. `\sagestr` takes
    // required `{code}`. No brace is not a span. There is no `*` form.
    ...
    // a span. There is no `*` form.
    // catchfile.sty leftover (GitHub #462): same three required braces
    // `{macro}{file}{setup}`. No optional trailing `[setup]`.
    if matches!(kind, VerbKind::Inputpython | VerbKind::CatchFileDef) {
...
enum VerbKind {
    ...
    /// `{macro}{file}{setup}` (catchfile.sty leftover; GitHub #462).
    CatchFileDef,
    ...
    /// a delimiter.
    EscVerb,
    /// fvextra leftover `\VerbatimInsertBuffer` / `\InsertBuffer` /
    /// `\VerbatimClearBuffer` (GitHub #468): optional `[...]`, no
    /// required brace. No `*` form.
    FvextraBuffer,
    /// fvextra leftover `\IterateBuffer` (GitHub #468): optional
...
/// catchfile.sty leftover `\CatchFileDef` / `\CatchFileEdef` (GitHub
/// #462). Three required braces `{macro}{file}{setup}`. Longer names
/// first so this is not CatchFile + leftover. No `*` form.
/// `\CatchFileBetweenTags` / `\CatchFileBetweenDelims` /
/// `\ExecuteMetaData` stay their own leftovers.
pub(crate) fn catchfile_leftover_cs_name(tail: &str) -> Option<&'static str> {
    for name in ["CatchFileEdef", "CatchFileDef"] {
        if let Some(after) = tail.strip_prefix(name) {
...
/// pythonhighlight.sty leftover `\pyth` (GitHub #450). Delimiter or
/// `{code}` body like `\lstinline`. Longer name first so it is not
/// `\py` + leftover. No `*` form (`\newcommand*`). `\inputpython` /
/// `\inputpythonfile` / `\py` stay their own leftovers.
...
/// leftover of a shorter name. Optional `[...]` then required
/// `{char}`. No `*` form. `\UseVerb` / `\Verb` stay their own leftovers.
pub(crate) fn fancyvrb_shortverb_cs_name(tail: &str) -> Option<&'static str> {
...
fn match_extra_verb_command<'a>(tail: &'a str, extras: &'a [String]) -> Option<&'a str> {
    ...
    for name in extras {
        if name.is_empty()
            ...
            || fancyvrb_shortverb_cs_name(name).is_some_and(|n| n == name.as_str())
            ...
            || catchfile_leftover_cs_name(name).is_some_and(|n| n == name.as_str())
            || name == "mint"
            || name == "Verb"
            || name == "SaveVerb"
            || name == "VerbatimInput"
...
mod tests {
    ...
    /// one span. `\VerbatimInsertBuffer[opt]` / `\InsertBuffer[opt]` /
    /// `\VerbatimClearBuffer` need no brace. `\IterateBuffer[opt]{cmd}`
    /// requires the brace. extras skip so a configured extra does not
    /// re-tokenize the no-brace form as Delim. `\UseVerb` stays its
    /// own leftover.
    #[test]
    fn latex_fvextra_buffer_leftover_cmds_stay_atomic() {
        assert_eq!(
        ...
        assert_eq!(
            split("See \\VerbatimInsertBuffer[foo] here. After."),
            vec![
                r"See \VerbatimInsertBuffer[foo] here.".to_string(),
                "After.".to_string()
            ]
        );
        assert_eq!(
            fvextra_buffer_leftover_cs_name("VerbatimInsertBuffer[foo]"),
            Some("VerbatimInsertBuffer"),
            "VerbatimInsertBuffer is this leftover"
        );
        assert_eq!(
            fvextra_buffer_leftover_cs_name("UseVerb{foo}"),
            None,
            "UseVerb stays its own leftover"
        );
        assert_eq!(
            fvextra_buffer_leftover_cs_name("VerbatimInput{foo}"),
            None,
            "VerbatimInput stays its own leftover"
        );
        assert_eq!(
            fvextra_buffer_leftover_cs_name("VerbatimInsertBufferX"),
            None,
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\VerbatimInsertBuffer*[foo]", 0, &[]),
            None,
            "VerbatimInsertBuffer has no star form"
        );
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\VerbatimInsertBuffer After.", 0, &extras),
            Some(r"\VerbatimInsertBuffer".len()),
            "configured extra VerbatimInsertBuffer must not re-tokenize the no-brace form as Delim"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\IterateBuffer[foo]{\do}", 0, &extras),
            Some(r"\IterateBuffer[foo]{\do}".len()),
    ...
    /// `\DefineShortVerb{\|}` / `\UndefineShortVerb{\|}` stay one span.
    /// Optional `[...]` then required `{char}`. extras skip so a
    /// configured extra does not re-tokenize the no-brace form as
    /// Delim. `\UseVerb` stays its own leftover. No `*` form.
    #[test]
    fn latex_fancyvrb_shortverb_cmds_stay_atomic() {
        assert_eq!(
            latex_verb_span_end_with(r"\DefineShortVerb{\|}", 0, &[]),
            Some(r"\DefineShortVerb{\|}".len()),
            "DefineShortVerb brace char is a leftover span"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\UndefineShortVerb{\|}", 0, &[]),
            Some(r"\UndefineShortVerb{\|}".len()),
            "UndefineShortVerb brace char is a leftover span"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\DefineShortVerb[commandchars=\\\{\}]{\|}", 0, &[]),
            Some(r"\DefineShortVerb[commandchars=\\\{\}]{\|}".len()),
            "DefineShortVerb optional keys then char brace is a leftover span"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\DefineShortVerb After.", 0, &[]),
            None,
            "DefineShortVerb without a brace char is not a verb span"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\UndefineShortVerb After.", 0, &[]),
            None,
            "UndefineShortVerb without a brace char is not a verb span"
        );
        assert_eq!(
            split(r"See \DefineShortVerb{\|} here. After."),
            vec![
        ...
        assert_eq!(
            fancyvrb_shortverb_cs_name("UndefineShortVerb{\\|}"),
            Some("UndefineShortVerb"),
            "UndefineShortVerb is the longer leftover name"
        );
        assert_eq!(
            fancyvrb_shortverb_cs_name("DefineShortVerb{\\|}"),
            Some("DefineShortVerb"),
            "DefineShortVerb is this leftover"
        );
        assert_eq!(
            fancyvrb_shortverb_cs_name("UseVerb{foo}"),
            None,
            "UseVerb stays its own leftover"
        );
        assert_eq!(
            fancyvrb_shortverb_cs_name("DefineShortVerbX"),
            None,
            "alphabetic leftover rejects a longer name"
        );
        assert_eq!(
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\UseVerb{foo}", 0, &[]),
            Some(r"\UseVerb{foo}".len()),
            "fancyvrb shortverb leftover must not steal UseVerb"
        );
    }

    /// Ticket fixture (GitHub #446): sagetex leftover inline
    /// `\sageplot[ltx opts][fmt]{graphics}` and `\sagestr{code}` stay
    /// one span. Following `After.` still splits. Longer names first
    /// so `\sageplot` / `\sagestr` / `\sageinput` are not `\sage` +
    /// leftover. extras skip so a configured extra does not
    /// re-tokenize the no-brace form as Delim. `\sage` stays the
    /// pythontex usefamily leftover. `\sageinput` stays its own span.
    #[test]
    fn latex_sagetex_inline_cmds_stay_atomic() {
        let plot = r"\sageplot{plot(sin(x))}";
        let text = r"See \sageplot{plot(sin(x))} here. After.";
        let (_, placeholders) = protect_inline_tokens(text);
        assert!(
            placeholders.iter().any(|p| p == plot),
            "sageplot span must be protected, got {placeholders:?}"
        );
        assert_eq!(
        ...
        let opts = r"\sageplot[width=.75\textwidth]{plot(sin(x), x, 0, 2*pi)}";
        assert_eq!(
            latex_verb_span_end_with(opts, 0, &[]),
            Some(opts.len()),
            "sageplot optional ltx opts must stay in the span"
        );
        let fmt = r"\sageplot[][png]{plot(sin(x), x, 0, pi)}";
        assert_eq!(
            latex_verb_span_end_with(fmt, 0, &[]),
            Some(fmt.len()),
            "sageplot optional fmt must stay in the span"
        );
        let both = r"\sageplot[height=4cm][png]{plot(sin(x))}";
        assert_eq!(
            latex_verb_span_end_with(both, 0, &[]),
            Some(both.len()),
            "sageplot both optional args must stay in the span"
        );
        assert_eq!(
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\sagestr {foo}", 0, &[]),
            Some(r"\sagestr {foo}".len()),
            "sagestr may skip space before the code brace"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("sagestr"),
            "sagestr must not be sage + leftover"
        );
        assert_eq!(
            sagetex_inline_cs_name("sage{1+1}"),
            None,
            "sage stays the pythontex usefamily leftover"
        );
        assert_eq!(
            sagetex_inline_cs_name("sageinput{foo.sage}"),
            None,
            "sageinput is not this leftover"
        );

        let extras_plot = ["sageplot".to_string()];
        assert_eq!(
            latex_verb_span_end_with(r"\sageplot After.", 0, &extras_plot),
            None,
            "configured extra sageplot must not re-tokenize the no-brace form as Delim"
        );
        assert_eq!(
            latex_verb_span_end_with(plot, 0, &extras_plot),
            Some(plot.len()),
            "configured extra sageplot must keep the brace form as leftover"
        );
        let extras_str = ["sagestr".to_string()];
        assert_eq!(
            latex_verb_span_end_with(r"\sagestr After.", 0, &extras_str),
            None,
            "configured extra sagestr must not re-tokenize the no-brace form as Delim"
        );
        assert_eq!(
            latex_verb_span_end_with(sagestr, 0, &extras_str),
            Some(sagestr.len()),
            "configured extra sagestr must keep the brace form as leftover"
        );

        assert_eq!(
            latex_verb_span_end_with(r"\sage{1+1}", 0, &[]),
            Some(r"\sage{1+1}".len()),
            "sage brace form must stay the usefamily leftover"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\sage|1+1|", 0, &[]),
            Some(r"\sage|1+1|".len()),
            "sage delimiter form must stay the usefamily leftover"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\sageinput{foo.sage}", 0, &[]),
            Some(r"\sageinput{foo.sage}".len()),
            "sageplot leftover must not steal sageinput"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\sagec{1+1}", 0, &[]),
            Some(r"\sagec{1+1}".len()),
            "sagetex inline must not steal usefamily sagec"
        );
        assert_eq!(
    ...
    /// Ticket fixture (GitHub #462): catchfile leftover `\CatchFileDef`
    /// / `\CatchFileEdef` stay one span; following `After.` still
    /// splits. extras skip the no-brace form. Longer names first so
    ...
    fn latex_catchfile_def_cmds_stay_atomic() {
        ...
        assert_eq!(
            split(r"See \CatchFileEdef{\foo}{foo.py}{} here. After."),
            vec![
                r"See \CatchFileEdef{\foo}{foo.py}{} here.".to_string(),
                "After.".to_string()
            ]
        );
        let tags = r"\CatchFileBetweenTags{\tmp}{foo.tex}{TAG}";
        assert_eq!(
            ...
            Some(tags.len()),
            "CatchFileBetweenTags must stay one leftover span"
        );
        assert_eq!(
        ...
        let extras = ["CatchFileDef".to_string(), "CatchFileEdef".to_string()];
        assert_eq!(
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\CatchFileEdef foo.py", 0, &extras),
            None,
