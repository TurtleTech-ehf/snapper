...
/// `{filename}`; no brace is not a span. `\VerbatimInput` /
/// `\BVerbatimInput` / `\LVerbatimInput` (fancyvrb.sty leftover;
/// GitHub #399) use the same optional `[...]` then `{filename}` walk.
/// Matched before `\Verb` so `\VerbatimInput` is not `\Verb` plus
/// leftover letters. `\PitonInputFile` (piton.sty leftover;
/// `d < > O { } m`; GitHub #406) takes optional `<...>`, optional
...
/// xparse `d < >`: one optional delimited group. First `>` closes;
/// not nested (piton.sty `\PitonInputFile`; GitHub #406).
...
mod tests {
    ...
    fn latex_pitoninputfile_stays_atomic() {
        ...
        assert_eq!(
            ...
            "PitonInputFile standalone angle spec must stay in the span"
        );
