...
pub(crate) fn latex_verb_span_end_with(
    ...
    // fancyvrb `\VerbatimInput` family before `\Verb` so the longer
    // names are not rejected as `\Verb` + alphabetic leftover (#399).
    ...
    // `\VerbatimInput[opts]{file}` (GitHub #399) are brace-only.
    // tools/verbatim.sty `\verbatiminput{file}` is the same brace file
    // arg with no optional `[...]` (GitHub #398).
...
/// `\LVerbatimInput` (`FV@Command`; GitHub #399). Matched before `\Verb`
/// so the longer names are not rejected as `\Verb` + alphabetic leftover.
/// verbatim.sty `\verbatiminput` is a different command and is not a span.
fn verbatiminput_cmd_len(tail: &str) -> Option<usize> {
...
mod tests {
    ...
    /// Ticket fixture (GitHub #399): fancyvrb `\VerbatimInput{file}`
    /// is one leftover command like `\lstinputlisting`; following
    /// `After.` still splits. B/L twins and optional `[...]` are the
    /// same class. `\Verb` / `\inputminted` / `\lstinputlisting` /
    /// verbatim.sty `\verbatiminput` are not stolen.
    #[test]
    fn latex_verbatiminput_stays_atomic() {
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\Verb|a.b!|", 0, &[]),
            Some(r"\Verb|a.b!|".len()),
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\verbatiminput{foo.py}", 0, &[]),
            Some(r"\verbatiminput{foo.py}".len()),
            "tools/verbatim.sty \\verbatiminput stays a span"
        );
    }

    /// Ticket fixture (GitHub #398): tools/verbatim.sty `\verbatiminput{file}`
    /// is one leftover token; following `After.` still splits.
    #[test]
    fn latex_tools_verbatiminput_stays_atomic() {
        let text = r"See \verbatiminput{foo.py} here. After.";
        let (_, placeholders) = protect_inline_tokens(text);
        assert!(
            placeholders.iter().any(|p| p == r"\verbatiminput{foo.py}"),
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\VerbatimInput{foo.py}", 0, &[]),
            Some(r"\VerbatimInput{foo.py}".len()),
            "verbatiminput must not steal VerbatimInput"
        );
    }
    ...
    /// Optional `[...]` and `d < >` stay in the span (A combined + B
    /// standalone angle). `\piton` / tools/verbatim.sty
    /// `\verbatiminput` / fancyvrb `\VerbatimInput` stay spans.
    #[test]
    ...
    /// splits. Optional `[index]` stays in the span. No `*` form.
    ...
    fn latex_inputsc_stays_atomic() {
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\inputsc[1]{foo}", 0, &[]),
            Some(r"\inputsc[1]{foo}".len()),
            "inputsc optional index must stay in the span"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\inputsc {foo}", 0, &[]),
            Some(r"\inputsc {foo}".len()),
            "inputsc may skip space before the name brace"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\inputsc foo", 0, &[]),
            None,
            "inputsc without a brace name arg is not a verb span"
        );
        ...
        assert_eq!(
            latex_verb_span_end_with(r"\inputpy{foo.py}", 0, &[]),
            Some(r"\inputpy{foo.py}".len()),
            "inputsc must not steal inputpy"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\listinginput{1}{foo.py}", 0, &[]),
            Some(r"\listinginput{1}{foo.py}".len()),
            "inputsc must not steal listinginput"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\sageinput{foo.sage}", 0, &[]),
            Some(r"\sageinput{foo.sage}".len()),
            "inputsc must not steal sageinput"
        );
        assert_eq!(
            latex_verb_span_end_with(r"\inputpylab{foo.py}", 0, &[]),
            Some(r"\inputpylab{foo.py}".len()),
            "inputsc must not steal inputpylab"
        );
        assert_eq!(
