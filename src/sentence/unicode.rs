...
static INLINE_TOKEN_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        &[
            ...
            // `<<<contents>>>`, contents = [^<>\n]+. Radio must precede
            // `<<...>>`: rust regex is leftmost-first, so the inner pair
            // would otherwise steal `<<<the Fourier. transform>>>`.
            r"<<<[^<>\n]+>>>",
...
/// Org `[cite...]`, Org `<<<radio>>>` / `<<target>>`, paired spans).
///
...
mod tests {
    ...
    fn inline_org_radio_target_interior_punct_is_not_a_sentence_boundary() {
        // GitHub #211 / snapper-gz10: org-radio-target-regexp `<<<...>>>`
        // must stay one token so an interior period is not a boundary.
        ...
        assert!(
            ...
            "radio must not collapse to an angle target, got {placeholders:?}"
        );
    ...
    fn inline_org_angle_target_interior_punct_is_not_a_sentence_boundary() {
        // Same class as radio: org-target-regexp `<<contents>>`.
