...
fn org_inline_src_or_call_span_end(text: &str, at: usize) -> Option<usize> {
    org_inline_src_span_end(text, at)
        .or_else(|| org_inline_call_span_end(text, at))
        .or_else(|| org_footnote_reference_span_end(text, at))
}
...
/// ends the object (org-element successor used `line-end-position`).
...
mod tests {
    ...
    fn nested_bracket_inline_org_footnote_is_one_token() {
        // org-element scan-lists on [] so `[fig. 1]` stays inside.
