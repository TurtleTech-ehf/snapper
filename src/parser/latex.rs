...
/// `\LUseVerbatim` / `\BUseVerbatim` / leftover `\Verb` /
/// `\EscVerb` spans.
...
/// Leftover fancyvrb replay / leftover `\Verb` / fvextra `\EscVerb`
/// (GitHub #457). One leftover walker for `\UseVerb` / `\UseVerb*` /
/// `\UseVerb[opt]{name}`, `\UseVerbatim` / `\LUseVerbatim` /
/// `\BUseVerbatim` `{name}`, leftover `\Verb` (kernel `\verb` stays
/// the #452 walker), and `\EscVerb` delimiter or `{code}`. Unicode
/// spans already exist for `\Verb`; this walker classifies an own-line
/// leftover as Structure so following flush prose does not join.
/// Mid-sentence spans stay Prose (unicode protected). Longer names
/// first so `\UseVerbatim` is not `\UseVerb` + leftover. Other leftover
/// walkers stay their own. `\SaveVerb` / `\verb` / `\piton` stay the
/// #452 walker. `\Piton{...}` is not a leftover user cmd (env only).
/// Other verb spans are skipped so `\verb|\UseVerb{x}|` is not stolen.
/// Walk stops at an unescaped `%` so a comment is not a command tail.
...
\end{document}";
    ...
    /// Ticket fixture (GitHub #457): leftover fancyvrb replay /
    /// leftover `\Verb` / fvextra leftover `\EscVerb` stay one
    /// Structure span. Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Mid-sentence `See \Verb|x| here.`
    /// still splits. Unicode spans stay protected. `\SaveVerb` /
    /// `\verb` / `\piton` stay atomic. extras skip so a configured
    ...
    fn fancyvrb_leftover_cmds_do_not_join_following_prose() {
        ...
        assert!(
            extras_out.contains("After.\nNext."),
            "configured extra must not re-tokenize the no-brace UseVerb form as Delim, got:\n{extras_out}"
        );
        let extras_esc =
            format_text("Before. Next.\n\\EscVerb After. Next.\n", &extras_cfg).unwrap();
        assert!(
            extras_esc.contains("After.\nNext."),
            "configured extra must not re-tokenize the no-brace EscVerb form as Delim, got:\n{extras_esc}"
        );
        let extras_delim = format_text(
            "Before. Next.\n\\EscVerb|print(1)|\nAfter. Next.\n",
            &extras_cfg,
