...
/// `sageblock`), pythontex.sty `pycode*` / `pyblock` / `pyblock*` /
/// `pyverbatim` / `pyverbatim*` / `pyconsole` / `pyconsole*` /
...
/// is the same `FV@Scan` class as `VerbatimOut` (GitHub #247). fancyvrb
...
\end{document}";
    ...
    /// `pyblock*` / `pyverbatim*` / `pyconsole*` are the starred twins
    /// of the unstarred `VerbatimEnvironment` names. Body stays Code;
