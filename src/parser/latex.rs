...
/// -- language is an option key on both listings.sty names.
...
/// expand; GitHub #230), listings.sty `lstlisting*` (`\lstnewenvironment`
/// sibling of `lstlisting`; same raw `\begin`/`\end` scan; GitHub #234),
...
\end{document}";
    ...
    /// starred sibling of `lstlisting`. Same raw scan. Body stays Code;
    ...
    fn lstlisting_star_is_code_not_prose() {
        ...
        let Some((header, body, footer)) = code else {
            panic!("lstlisting* must be Code, got: {regions:?}");
        };
        assert!(
            header.contains(r"\begin{lstlisting*}"),
            "header must be \\begin{{lstlisting*}}, got header={header:?}"
        );
        assert!(
            body.contains("First line. Second line."),
            "lstlisting* body must be Code, got body={body:?}"
        );
        assert!(
            footer.contains(r"\end{lstlisting*}"),
            "footer must be \\end{{lstlisting*}}, got footer={footer:?}"
        ...
        assert!(
            ...
            "prose after lstlisting* must resume, got: {regions:?}"
        );
        let out = format_text(input, &latex_cfg()).unwrap();
        assert!(
        ...
        // Same raw scan as lstlisting: `%` does not hide `\end{lstlisting*}`.
        let raw = concat!(
            "\\begin{lstlisting*}\n",
            "print(1) % \\end{lstlisting*}\n",
            "After the listing. Next.\n",
        ...
        let (raw_body, raw_footer) =
            raw_code.expect(&format!("lstlisting* must be Code, got: {raw_regions:?}"));
        assert!(
            !raw_body.contains("After the listing"),
            "% must not hide \\end{{lstlisting*}}; after-text is not listing body, got body={raw_body:?}"
        );
        assert!(
            raw_footer.contains(r"\end{lstlisting*}"),
            "footer must be \\end{{lstlisting*}} even after %, got footer={raw_footer:?}"
        );

        let lang_input = "\\begin{lstlisting*}[language=Python]\nprint(1)\n\\end{lstlisting*}\n";
        let lang_regions = LatexParser::default().parse(lang_input);
        assert!(
            lang_regions.iter().any(|r| matches!(
                r,
                Region::Code { lang: Some(lang), .. } if lang == "Python"
            )),
            "lstlisting* language= must set Code lang, got: {lang_regions:?}"
        );
    }
