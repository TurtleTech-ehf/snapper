...
impl LatexParser {
    ...
    /// `\listinginput` / `\listingcont` / `\verbatimtabinput` / `\verbatimwrite` / `\sageinput` /
    /// `\inputsc` / `\Scontents` / `\typestored` / `\getstored` /
    /// `\mergesc` / `\meaningsc` / `\foreachsc` / `\pythontexcustomc` /
    /// `\pyth` / `\UseVerb` / `\UseVerbatim` / `\LUseVerbatim` /
    /// `\BUseVerbatim` / `\DefineShortVerb` / `\UndefineShortVerb` /
    /// `\EscVerb` / `\VerbatimInsertBuffer` /
...
/// `\CatchFileEdef` / `\listinginput` / `\listingcont` /
/// `\verbatimtabinput` /
...
/// spans are skipped so `\verb|\listingcont|` is not stolen. Walk stops
/// at an unescaped `%` so a comment is not a command tail. There is no
/// `*` form. `\listinginput` stays its own leftover.
...
/// (optional `[tabwidth]`, required `{filename}`; GitHub #465). Other
/// verb spans are skipped so `\verb|\verbatimtabinput{x}|` is not
/// stolen. Walk stops at an unescaped `%` so a comment is not a
/// command tail. Matched as its own leftover so `\verbatiminput` does
/// not reject this longer name.
...
/// Leftover moreverb `\verbatimwrite` / `\verbatimwrite*`
/// (required `{file}`; GitHub #471). One leftover walker for both
/// names. Other verb spans are skipped so `\verb|\verbatimwrite{x}|`
/// is not stolen. Walk stops at an unescaped `%` so a comment is not
/// a command tail. `\verbatimtabinput` / `\listinginput` /
/// `\verbatiminput` stay their own leftovers.
fn find_verbatimwrite_at(line: &str, from: usize, extra_cmds: &[String]) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, verbatimwrite_cs_at)
}

fn verbatimwrite_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
    let Some(after) = tail.strip_prefix("verbatimwrite") else {
        return false;
    };
    !after.starts_with(|c: char| c.is_ascii_alphabetic())
}
...
/// `\sageplot` / `\sagestr` are the leftover inline walker (GitHub #446).
...
/// Leftover sagetex.sty inline cmds (GitHub #446). One leftover
/// walker for `\sageplot` (optional `[ltx opts][fmt]`, required
/// `{graphics}`) and `\sagestr` (required `{code}`). Longer names
/// first so `\sageplot` / `\sagestr` / `\sageinput` are not `\sage` +
/// leftover. `\sage` stays the pythontex usefamily leftover (GitHub
/// #445). Other verb spans are skipped so `\verb|\sageplot{x}|` is
/// not stolen. Walk stops at an unescaped `%` so a comment is not a
/// command tail. There is no `*` form.
...
/// Leftover scontents.sty `\Scontents` / `\Scontents*` (`!s !O{}` then
/// standard or verbatim arg) and `\typestored` / `\getstored` /
/// `\mergesc` / `\meaningsc` / `\foreachsc` (`o m` / `O{-1} m`; GitHub
/// #451). Longer names first. Other verb spans are skipped so
/// `\verb|\Scontents{x}|` is not stolen. Walk stops at an unescaped `%`
/// so a comment is not a command tail. `\inputsc` stays its own leftover.
/// `\newenvsc` is a constructor, not this leftover. Only `\Scontents`
/// has a `*` form.
fn find_scontents_leftover_at(
    line: &str,
    from: usize,
    extra_cmds: &[String],
) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, scontents_leftover_cs_at)
}

fn scontents_leftover_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
    scontents_leftover_cs_name(tail).is_some()
}

/// Leftover filename-input / verbatim-input tail (GitHub #439 / #443).
/// One leftover walker for pythontex.sty `\inputpygments` / `\pygment`,
/// pythonhighlight.sty `\inputpython` / `\inputpythonfile`, and
/// catchfilebetweentags.sty `\CatchFileBetweenTags` /
/// `\CatchFileBetweenDelims` / `\ExecuteMetaData`. Longer
...
/// required `{type}`, then delimiter or `{code}`; GitHub #449). Longer
/// name first so it is not `\py` + leftover. Other verb spans are
/// skipped so `\verb|\pythontexcustomc{python}{x}|` is not stolen.
/// Walk stops at an unescaped `%` so a comment is not a command tail.
/// `\py` / `\inputpy` stay their own leftovers. There is no `*` form.
/// `\setpythontexcustomcode` is preamble-only.
fn find_pythontexcustomc_at(
    line: &str,
    from: usize,
    extra_cmds: &[String],
) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, pythontexcustomc_cs_at)
}

fn pythontexcustomc_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
...
/// Leftover pythonhighlight.sty `\pyth` (delimiter or `{code}` body like
/// `\lstinline`; GitHub #450). Longer name first so it is not `\py` +
/// leftover. Other verb spans are skipped so `\verb|\pyth{x}|` is not
/// stolen. Walk stops at an unescaped `%` so a comment is not a
/// command tail. `\inputpython` / `\inputpythonfile` / `\py` stay their
/// own leftovers. There is no `*` form (`\newcommand*`).
...
/// One leftover walker for `\py` / `\pyc` / `\pys` / `\pyb` / `\pyv`,
/// `\pycon` and twins, `\sympy` / `\pylab` and twins, plus option-family
/// `{name}` / `{name}c` / `{name}s` / `{name}b` / `{name}v` (ruby / rb /
/// julia / jl / matlab / octave / bash / sage / rust / rs / R / perl /
/// pl / perlsix / psix / javascript / js). Longer names first so
/// `\pycon` / `\pylab` / `\pyc` / `\rubyc` are not a shorter name +
/// leftover. Other verb spans are skipped so `\verb|\py{x}|` is not
/// stolen. Walk stops at an unescaped `%` so a comment is not a
/// command tail. `\inputpy` / `\inputpygments` / `\pygment` stay
/// their own leftovers. `juliacon` / `Rcon` mint envs via
/// `\makepythontexfamily@con`, not these inlines.
fn find_pytx_inline_at(line: &str, from: usize, extra_cmds: &[String]) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, pytx_inline_cs_at)
}

fn pytx_inline_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
...
/// (GitHub #473). One leftover walker for both names.
/// `\DefineShortVerb` takes optional `[...]` then required `{char}`;
/// `\UndefineShortVerb` takes required `{char}`. Other verb spans are
/// skipped so `\verb|\DefineShortVerb{\|}|` is not stolen. Walk stops
/// at an unescaped `%` so a comment is not a command tail. `\UseVerb`
/// / `\Verb` stay their own leftovers. No `*` form.
fn find_fancyvrb_shortverb_leftover_at(
    line: &str,
    from: usize,
    extra_cmds: &[String],
) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, fancyvrb_shortverb_leftover_cs_at)
}

fn fancyvrb_shortverb_leftover_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
    fancyvrb_shortverb_leftover_cs_name(tail).is_some()
}
...
fn piton_brace_leftover_end(line: &str, at: usize) -> Option<usize> {
    ...
    let name = verb_span_leftover_cs_name(tail)?;
    if name != "piton" {
        return None;
    }
    let mut i = at + 1 + name.len();
        ...
        while i < code.len() {
            ...
            if let Some((start, end)) =
                find_lstinputlisting_at(code, i, &self.parser.extra_verbatim_commands)
                    ...
                    .or_else(|| {
                        find_scontents_leftover_at(code, i, &self.parser.extra_verbatim_commands)
                    })
                    .or_else(|| {
                    ...
                    .or_else(|| {
                        find_fancyvrb_shortverb_leftover_at(
                            code,
                            i,
                            &self.parser.extra_verbatim_commands,
                        )
...
\end{document}";
    ...
    /// `\pythontexcustomc[begin|end]{type}{code}` stays one Structure
    /// span. Following flush `After.` does not join. `After.` / `Next.`
    /// still split. Longer name first so it is not `\py` + leftover.
    /// `\py` / `\inputpy` / `pythontexcustomcode` env unchanged. extras
    ...
    fn pythontexcustomc_does_not_join_following_prose() {
        ...
        let opts_out = format_text(opts, &latex_cfg()).unwrap();
        assert!(
            opts_out.contains("\\pythontexcustomc[begin]{python}{import numpy}\n"),
            "pythontexcustomc optional begin must stay atomic, got:\n{opts_out}"
        );
        assert!(
            !opts_out.contains("\\pythontexcustomc[begin]{python}{import numpy} After."),
        ...
        assert!(
            delim_out.contains("\\pythontexcustomc{python}|import numpy|\n"),
            "pythontexcustomc delimiter body must stay atomic, got:\n{delim_out}"
        );
        assert!(
            !delim_out.contains("\\pythontexcustomc{python}|import numpy| After."),
            "delimiter pythontexcustomc must not join following prose, got:\n{delim_out}"
        );
    ...
    /// `\pyth{code}` / `\pyth|code|` stays one Structure span. Following
    /// flush `After.` does not join. `After.` / `Next.` still split.
    /// Longer name first so it is not `\py` + leftover. `\inputpython`
    /// / `\inputpythonfile` / `\pythontexcustomc` / `\py` unchanged.
    ...
    fn pyth_does_not_join_following_prose() {
        ...
        assert!(
            delim_out.contains("After.\nNext."),
            "prose after delimiter pyth must still split, got:\n{delim_out}"
        );
    ...
    /// `After.` / `Next.` still split. Longer names first.
    /// `\inputsc` / scontents env unchanged. extras skip so a
    ...
    /// split. `\lstinputlisting` / `\inputpy` unchanged. extras skip so
    ...
    fn inputpython_does_not_join_following_prose() {
        ...
        assert!(
            extras_brace.contains("After.\nNext."),
            "configured extra inputpython brace form must still split following prose, got:\n{extras_brace}"
        );
    }

    /// Ticket fixture (GitHub #446): sagetex leftover inline
    /// `\sageplot[ltx opts][fmt]{graphics}` and `\sagestr{code}` stay
    /// one Structure span. Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Longer names first so
    /// `\sageplot` / `\sagestr` / `\sageinput` are not `\sage` +
    /// leftover. `\sage` stays the pythontex usefamily leftover.
    /// `\sageinput` unchanged. extras skip so a configured extra does
    ...
    /// `\sympy` / `\pylab` and twins stay one Structure span (brace or
    /// `|delim|` body). Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Longer names first so `\pycon`
    /// / `\pylab` / `\pyc` are not `\py` + leftover. `\inputpy` /
    ...
    fn pytx_inline_cmds_do_not_join_following_prose() {
        ...
        for name in [
            ...
            let delim_out = format_text(&delim, &latex_cfg()).unwrap();
            assert!(
                delim_out.contains(&format!("\\{name}|print(1)|\n")),
                "{name} delimiter body must stay atomic, got:\n{delim_out}"
            );
            assert!(
                !delim_out.contains(&format!("\\{name}|print(1)| After.")),
                "delimiter {name} must not join following prose, got:\n{delim_out}"
            );
            assert!(
                delim_out.contains("After.\nNext."),
                "prose after delimiter {name} must still split, got:\n{delim_out}"
            );
        }
        ...
        let extras_cfg = FormatConfig {
            format: crate::format::Format::Latex,
            latex_verbatim_commands: vec!["py".to_string()],
            ..Default::default()
        }
        .without_safety_backstops();
        let extras_out = format_text("Before. Next.\n\\py After. Next.\n", &extras_cfg).unwrap();
        assert!(
            extras_out.contains("After.\nNext."),
            "configured extra py must not re-tokenize the no-body form as Delim, got:\n{extras_out}"
        );
        let extras_brace =
            format_text("Before. Next.\n\\py{print(1)}\nAfter. Next.\n", &extras_cfg).unwrap();
        assert!(
            extras_brace.contains("\\py{print(1)}\n"),
            "configured extra py must keep the brace form as leftover, got:\n{extras_brace}"
        );
        assert!(
            extras_brace.contains("After.\nNext."),
    ...
    /// `usefamily` inline cmds stay one Structure span (brace or
    /// `|delim|` body). Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Longer names first so `\rubyc`
    /// is not `\ruby` + leftover. `\py` / `\inputpy` / `\inputpython`
    /// unchanged. extras skip so a configured extra does not
    /// re-tokenize the no-body form as Delim. `juliacon` / `Rcon`
    ...
    fn pytx_usefamily_inline_cmds_do_not_join_following_prose() {
        ...
        assert!(
            ruby_out.contains("After.\nNext."),
            "prose after ruby must still split, got:\n{ruby_out}"
        );
        ...
        let rubyc = concat!("Before. Next.\n", "\\rubyc{puts 1}\n", "After. Next.\n",);
        let rubyc_out = format_text(rubyc, &latex_cfg()).unwrap();
        assert!(
            rubyc_out.contains("\\rubyc{puts 1}\n"),
            "rubyc must not be ruby + leftover, got:\n{rubyc_out}"
        );
        assert!(
            !rubyc_out.contains("\\rubyc{puts 1} After."),
            "rubyc must not join following prose, got:\n{rubyc_out}"
        );
        ...
        let extras_brace =
            format_text("Before. Next.\n\\ruby{puts 1}\nAfter. Next.\n", &extras_cfg).unwrap();
        assert!(
            extras_brace.contains("\\ruby{puts 1}\n"),
            "configured extra ruby must keep the brace form as leftover, got:\n{extras_brace}"
        );
        assert!(
            extras_brace.contains("After.\nNext."),
    ...
    /// / `\verbatimwrite*` stay one Structure span. Following flush
    /// `After.` does not join. `After.` / `Next.` still split.
    /// verbatimtabinput / listinginput stay atomic. extras skip so a
    /// configured extra does not re-tokenize the no-brace form as Delim.
    #[test]
    fn verbatimwrite_does_not_join_following_prose() {
        ...
        let tab_out = format_text(tab, &latex_cfg()).unwrap();
        assert!(
            tab_out.contains("\\verbatimtabinput{foo.py}\n"),
            "verbatimtabinput must stay unchanged, got:\n{tab_out}"
        );
        assert!(
            !tab_out.contains("\\verbatimtabinput{foo.py} After."),
            "verbatimtabinput must not join following prose, got:\n{tab_out}"
        );
        ...
        let extras_cfg = FormatConfig {
            format: crate::format::Format::Latex,
            latex_verbatim_commands: vec!["verbatimwrite".to_string()],
            ..Default::default()
        }
        .without_safety_backstops();
        let extras_out =
            format_text("Before. Next.\n\\verbatimwrite After. Next.\n", &extras_cfg).unwrap();
        assert!(
            extras_out.contains("After.\nNext."),
            "configured extra verbatimwrite must not re-tokenize the no-brace form as Delim, got:\n{extras_out}"
        );
        let extras_brace = format_text(
            "Before. Next.\n\\verbatimwrite{foo.py}\nAfter. Next.\n",
            &extras_cfg,
        )
        .unwrap();
        assert!(
            extras_brace.contains("\\verbatimwrite{foo.py}\n"),
            "configured extra verbatimwrite must keep the brace form as leftover, got:\n{extras_brace}"
        );
        assert!(
            extras_brace.contains("After.\nNext."),
    ...
    /// Ticket fixture (GitHub #476): moreverb leftover `\listingcont`
    /// stays one Structure span. Following flush `After.` does not
    /// join. `After.` / `Next.` still split. `\listinginput` stays
    /// atomic. extras skip so a configured extra does not re-tokenize
    /// the no-brace form as Delim.
    #[test]
    fn listingcont_does_not_join_following_prose() {
        ...
        assert!(
            regions.iter().any(|r| matches!(
                r,
                Region::Structure(s) if s.contains(r"\listingcont")
            )),
            "listingcont must stay one Structure command, got: {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
                Region::Prose(p) if p.contains(r"\listingcont")
            )),
            "listingcont must not leak into Prose, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
        ...
        let out = format_text(input, &latex_cfg()).unwrap();
        assert!(
            out.contains("\\listingcont\n"),
            "listingcont must stay one atomic command, got:\n{out}"
        );
        assert!(
            !out.contains("\\listingcont After."),
            "following flush prose must not join the command line, got:\n{out}"
        );
        assert!(
            out.contains("Before.\nNext."),
            "prose before listingcont must still split, got:\n{out}"
        );
        assert!(
            out.contains("After.\nNext."),
            "prose after listingcont must still split, got:\n{out}"
