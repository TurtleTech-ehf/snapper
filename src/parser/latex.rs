...
mod tests {
    ...
    fn koma_addsec_addchap_addpart_optional_short_title_is_structure() {
        ...
        let cmds = [r"\addsec", r"\addchap", r"\addpart"];
        for cmd in cmds {
            let input = format!(
        ...
        let input =
            "\\addsec[Short. Title.]{A long title. With two sentences.}\nBody. More body.\n";
        let cfg = FormatConfig {
            format: Format::Latex,
            ..Default::default()
        }
        ...
        let out = format_text(input, &cfg).unwrap();
        assert!(
            out.contains("\\addsec[Short. Title.]{A long title. With two sentences.}"),
            "KOMA addsec line must stay one line, got:\n{out}"
        );
        assert!(
            out.contains("Body.\nMore body."),
            "body after KOMA addsec must still reflow, got:\n{out}"
        );
        assert_eq!(format_text(&out, &cfg).unwrap(), out);
    }
