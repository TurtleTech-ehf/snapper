...
use crate::sentence::unicode::{
    ...
    sagetex_inline_cs_name, scontents_leftover_cs_name,
};
...
/// piton.sty `\piton`. Unicode spans already exist; a flush command
/// (only whitespace before and after) is Structure so following flush
/// prose does not join. Mid-sentence spans stay Prose. Longer names
/// first so `\mintinline` is not `\mint` + leftover. Other verb spans
/// are skipped so `\verb|\lstinline{x}|` is not stolen. Walk stops at
/// an unescaped `%` so a comment is not a command tail.
/// `\lstinputlisting` / `\inputminted` / `\PitonInputFile` stay their
/// own leftovers. `\piton{...}` is the generic cmd-arg form
/// (GitHub #305), not a verb delimiter.
    ...
    while i < stop {
        if bytes[i] == b'\\' {
            if verb_span_leftover_cs_at(line, i) {
                if let Some(end) = latex_verb_span_end_with(line, i, extra_cmds)
                    .or_else(|| piton_brace_span_end(line, i))
                {
...
/// piton.sty brace form is not a verb delimiter (GitHub #305). Walk
/// the generic `\cmd{arg}` closer so `\piton{print(1)}` is one leftover
/// Structure span.
fn piton_brace_span_end(line: &str, at: usize) -> Option<usize> {
    ...
    while i < line.len() && matches!(line.as_bytes()[i], b' ' | b'\t') {
        i += 1;
    }
...
\end{document}";
    ...
    /// Ticket fixture (GitHub #452): leftover verb-span cmds stay one
    /// Structure span. Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Mid-sentence spans stay
    /// protected. extras skip unchanged. `\py` / `\sageplot` stay
    /// their own leftovers.
    #[test]
    fn verb_span_leftover_cmds_do_not_join_following_prose() {
        ...
        assert!(
            ...
            "mid-sentence verb must stay intact and still split After., got:\n{mid_out}"
        );
        ...
        let extras_cfg = FormatConfig {
            format: crate::format::Format::Latex,
            latex_verbatim_commands: vec!["mintinline".to_string()],
            ..Default::default()
        }
        .without_safety_backstops();
        let extras_out =
        ...
        assert!(
            extras_out.contains("After.\nNext."),
            "configured extra mintinline must not re-tokenize the no-lang form as Delim, got:\n{extras_out}"
        );
        let extras_span = format_text(
            "Before. Next.\n\\mintinline{python}|print(1)|\nAfter. Next.\n",
            &extras_cfg,
        )
        .unwrap();
        assert!(
            extras_span.contains("\\mintinline{python}|print(1)|\n"),
            "configured extra mintinline must keep the span form as leftover, got:\n{extras_span}"
        );
        assert!(
            extras_span.contains("After.\nNext."),
            "configured extra mintinline span form must still split following prose, got:\n{extras_span}"
        );
    }
