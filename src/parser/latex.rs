...
/// moreverb `boxedverbatim`, tcolorbox `tcblisting` / `codeexample`,
/// standard `alltt` (alltt.sty: macros still apply, line breaks stay
/// raw), spverbatim.sty `spverbatim` (raw body; `\spverb` is the
/// matching delimiter-body command, GitHub #235), and the `comment`
/// package env (tree-sitter `comment_environment`: raw through
...
/// (GitHub #213). fvextra `VerbatimWrite` is the same `FV@Scan` class
/// as `VerbatimOut` (GitHub #247). fancyvrb `Verbatim*` / `BVerbatim*`
/// / `LVerbatim*` are the starred twins of those names (same
/// `\FV@Scan`; GitHub #244). `alltt` is the same verbatim-like class
/// (GitHub #230). listings.sty `\lstnewenvironment{lstlisting}` also
/// defines `lstlisting*` (same raw body scan; GitHub #234).
...
\end{document}";
    ...
    /// FV@Scan class as VerbatimOut. The required `{file}` argument
