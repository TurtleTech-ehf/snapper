...
impl LatexParser {
    ...
    /// `\inputpycon` / configured verbatim commands.
...
/// required `{file}`; GitHub #419). One leftover walker for both
/// names. `inputpycon` before `inputpy` so the longer name is not
/// stolen; alphabetic tail rejects `\inputpygments`. Other verb spans
/// are skipped so `\verb|\inputpy{x}|` is not stolen. Walk stops at
