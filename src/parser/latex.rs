...
impl LatexParser {
    ...
    /// `\inputpy` / `\inputpycon` / `\listinginput` / configured verbatim commands.
...
/// `\VerbatimInput` / `\inputpy` / `\inputpycon` / `\listinginput` spans.
...
/// `{start}`, required `{file}`; GitHub #424). Other verb spans are
/// skipped so `\verb|\listinginput{1}{x}|` is not stolen. Walk stops
/// at an unescaped `%` so a comment is not a command tail. One brace
/// is not a span so `\listing{1}` / incomplete input is not stolen.
...
\end{document}";
    ...
    /// join. listing / listingcont / verbatiminput / inputpy /
    ...
    fn listinginput_does_not_join_following_prose() {
        ...
        assert!(
            ...
            "listing env must stay a code env, got:\n{listing_out}"
        );
        assert!(
            listing_out.contains("After the block.\nNext."),
            "prose after listing env must still split, got:\n{listing_out}"
        );
        ...
        assert!(
            ...
            "listingcont env must stay a code env, got:\n{listingcont_out}"
        );
