...
/// delimiter-body command, GitHub #235), and the `comment` package env
/// (tree-sitter `comment_environment`: raw through matching
...
/// (GitHub #213). fancyvrb `Verbatim*` / `BVerbatim*` / `LVerbatim*`
/// are the starred twins of those names (same `\FV@Scan`; GitHub
/// #244). `alltt` is the same verbatim-like class (GitHub #230).
/// listings.sty `\lstnewenvironment{lstlisting}` also defines
/// `lstlisting*` (same raw body scan; GitHub #234). tcolorbox listings
/// `tcblisting*` is the starred twin of `tcblisting` (GitHub #246).
...
\end{document}";
    ...
    /// Ticket fixture (GitHub #246): tcolorbox listings `tcblisting*` is
    /// the starred twin of `tcblisting`. Required `{listing only}` stays
    /// on the begin header. Body stays Code; following prose still splits.
    #[test]
    fn tcolorbox_tcblisting_star_is_code_not_prose() {
        ...
        assert!(
            ...
            "tcblisting* required arg must stay on the begin header, got header={header:?}"
        );
        assert!(
            body.contains("First line. Second line."),
        ...
        assert!(
            ...
            "tcblisting* footer must be \\end{{tcblisting*}}, got footer={footer:?}"
        );
        assert!(
            !regions
                .iter()
