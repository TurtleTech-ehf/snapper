...
fn display_math_open_at(s: &str, extra_cmds: &[String]) -> Option<(usize, DisplayMathDelim)> {
    if let Some(pos) = find_unescaped_display_bracket(s, 0, b'[', extra_cmds) {
        let t = s.trim_start();
        let lead = s.len() - t.len();
        if t.starts_with("$$") && lead <= pos {
            return Some((lead, DisplayMathDelim::Dollars));
        }
        return Some((pos, DisplayMathDelim::Bracket));
    }
    let t = s.trim_start();
    if t.starts_with("$$") {
        Some((s.len() - t.len(), DisplayMathDelim::Dollars))
    } else {
        None
    }
}
    ...
    fn consume_code_span(&mut self, code: &str, line: Line<'_>) -> bool {
        ...
        while i < code.len() {
                ...
                display_math_open_at(rest, &self.parser.extra_verbatim_commands)
            {
                ...
                let glue = if rest[..rel].trim().is_empty() {
                    0
                } else {
                    let prefix = &rest[..rel];
                    prefix.len() - prefix.trim_end_matches([' ', '\t']).len()
                };
                let open_rel = rel - glue;
