...
\end{document}";
    ...
    /// is one leftover command. Following flush prose does not join the
    /// command line. `After.` / `Next.` still split. B/L twins and
    ...
    fn verbatiminput_does_not_join_following_prose() {
        ...
        for name in ["VerbatimInput", "BVerbatimInput", "LVerbatimInput"] {
    ...
    /// and `d < >` stay atomic. Piton env, `\piton`, and fancyvrb
    /// `\VerbatimInput` unchanged. tools/verbatim.sty `\verbatiminput`
    /// is not this walker.
    #[test]
    ...
    /// `\inputpycon{file}` stay one leftover command. Optional `[...]`
    /// stays atomic. Following flush `After.` does not join.
    /// `inputpygments` is not stolen. pycode / pyconsole /
    /// `inputminted` / `lstinputlisting` / `tcbinputlisting` /
    ...
    fn inputpy_does_not_join_following_prose() {
        ...
        assert!(
            ...
            "inputpygments must not be stolen by inputpy, got:\n{pygments_out}"
        );
        assert!(
