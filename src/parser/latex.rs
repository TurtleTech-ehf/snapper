...
impl LatexParser {
    ...
    /// `\inputpylabcon` / `\inputsympy` / `\inputsympycon` /
...
/// Leftover pythontex.sty `\inputpy` family (optional `[...]`,
/// required `{file}`; GitHub #419 / #433). One leftover walker for
/// `inputpy` / `inputpycon` and pylab/sympy twins. Longer names first
/// so `\inputpylab` is not `\inputpy` + leftover. Other verb spans are
...
\end{document}";
    ...
    /// Optional `[...]` stays atomic. Following flush `After.` does
    /// not join. inputpy / listinginput / sageinput unchanged.
    #[test]
