...
use crate::sentence::unicode::{
    catchfile_leftover_cs_name, fancyvrb_leftover_cs_name, fvextra_buffer_cs_name,
...
/// Leftover fvextra buffer cmds (GitHub #468). One leftover walker for
...
/// stops at an unescaped `%` so a comment is not a command tail.
/// Longer names first so `\VerbatimInsertBuffer` is not
/// `\VerbatimInput` + leftover. `\UseVerb` / `\VerbatimInput` stay
/// their own leftovers.
fn find_fvextra_buffer_at(
    line: &str,
    from: usize,
    extra_cmds: &[String],
) -> Option<(usize, usize)> {
    find_leftover_cmd_at(line, from, extra_cmds, fvextra_buffer_cs_at)
}

fn fvextra_buffer_cs_at(line: &str, at: usize) -> bool {
    let Some(tail) = line.get(at..).and_then(|s| s.strip_prefix('\\')) else {
        return false;
    };
    fvextra_buffer_cs_name(tail).is_some()
}
        ...
        while i < code.len() {
            ...
            if let Some((start, end)) =
                find_lstinputlisting_at(code, i, &self.parser.extra_verbatim_commands)
                    ...
                    .or_else(|| {
                        find_fvextra_buffer_at(code, i, &self.parser.extra_verbatim_commands)
                    })
            {
...
\end{document}";
    ...
    /// `After.` / `Next.` still split. `\UseVerb` stays atomic.
    /// extras skip so a configured extra does not re-tokenize the
    /// no-brace form as Delim.
    #[test]
    fn fvextra_buffer_cmds_do_not_join_following_prose() {
        use crate::{FormatConfig, format_text};
        ...
        let extras_clear = format_text(
            "Before. Next.\n\\VerbatimClearBuffer After. Next.\n",
            &extras_cfg,
        )
        .unwrap();
        assert!(
            extras_clear.contains("After.\nNext."),
            "configured extra VerbatimClearBuffer must not re-tokenize the no-brace form as Delim, got:\n{extras_clear}"
        );
        let extras_brace = format_text(
        ...
        let extras_opt = format_text(
            "Before. Next.\n\\VerbatimInsertBuffer[foo]\nAfter. Next.\n",
            &extras_cfg,
        )
        .unwrap();
        assert!(
            extras_opt.contains("\\VerbatimInsertBuffer[foo]\n"),
            "configured extra VerbatimInsertBuffer must keep the optional form as leftover, got:\n{extras_opt}"
        );
        assert!(
            extras_opt.contains("After.\nNext."),
            "configured extra VerbatimInsertBuffer optional form must still split following prose, got:\n{extras_opt}"
        );
    }
