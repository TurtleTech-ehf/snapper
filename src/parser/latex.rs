...
/// still apply, line breaks stay raw), spverbatim.sty `spverbatim`
...
/// is the same `FV@Scan` class as `VerbatimOut` (GitHub #247).
/// fancyvrb `Verbatim*` / `BVerbatim*` / `LVerbatim*` are the starred
/// twins of those names (same `\FV@Scan`; GitHub #244). `alltt` is the
...
/// listing body; GitHub #246). moreverb `verbatimtab` is the same
/// tab-expanding raw class as `boxedverbatim` (GitHub #250).
