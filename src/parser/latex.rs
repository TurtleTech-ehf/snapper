...
/// Leftover filename-input / verbatim-input tail (GitHub #439 / #443 / #462).
/// One leftover walker for pythontex.sty `\inputpygments` / `\pygment`,
/// pythonhighlight.sty `\inputpython` / `\inputpythonfile`,
/// catchfilebetweentags.sty `\CatchFileBetweenTags` /
/// `\CatchFileBetweenDelims` / `\ExecuteMetaData`, and catchfile.sty
/// leftover `\CatchFileDef` / `\CatchFileEdef`. Longer
/// `inputpythonfile` first so it is not `inputpython` + leftover.
/// Longer `CatchFile*` names first so this is not `CatchFile` + leftover.
...
\end{document}";
    ...
    /// `\CatchFileDef{\macro}{file}{setup}` and `\CatchFileEdef` stay
    /// one Structure span. Following flush `After.` does not join.
    /// `After.` / `Next.` still split. Longer names first so this is
    /// not `CatchFile` + leftover. `\CatchFileBetweenTags` stays
