...
///
/// `comment` is the `comment` package env (tree-sitter `comment_environment`):
/// raw through matching `\end{comment}`, not prose.
fn is_builtin_code_env(name: &str) -> bool {
    matches!(name, "minted" | "lstlisting" | "verbatim" | "comment")
}
    ...
    fn consume_code_span(&mut self, code: &str, line: Line<'_>) -> bool {
        ...
        while i < code.len() {
            ...
            if let Some(open) = find_iffalse_at(code, i, &self.parser.extra_verbatim_commands) {
                self.append_prose_slice(line.start + i, &code[i..open]);
                self.flush();
