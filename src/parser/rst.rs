...
fn parse_line_based(input: &str) -> Vec<SpannedRegion> {
    ...
    while i < total {
        ...
        // / header / footer / line-block) nested-parse their body: the
        // opener and `:option:` fields stay Structure; the body hangs
        // and reflows like a block quote. Opaque names keep the old
        // freeze (GitHub #54 / #351 / #386 / #422). A flush paragraph
        // after a compact container body is a new paragraph, not more
        // note (GitHub #344). Specific admonitions nested-parse same-line
        // text after `::` as the first body paragraph: marker Structure,
        // body hung Prose that still splits (GitHub #349). SubstitutionDef
        // `.. |name| replace::` is the same leftover: the replace body
        // is a nested-parsed paragraph (GitHub #417). header / footer
        // take no argument; same-line text after `::` is leftover Prose
        // (GitHub #422). `parsed-literal` takes no argument; same-line
        // text after `::` is leftover Prose (GitHub #426). `line-block`
        // takes no argument; same-line text after `::` is leftover Prose
        // (GitHub #430). `| ` line-blocks already end at the first flush
        // paragraph (#409). `meta` takes no argument; its body is a
        // field list whose values are leftover paragraphs (GitHub #434).
...
mod tests {
    ...
    fn leftover_parsed_literal_fixture_hangs_and_splits() {
        ...
        assert_eq!(
            ...
            "parsed-literal same-line body is leftover Prose after the marker"
        );
    ...
    fn leftover_parsed_literal_same_line_marker_is_structure_body_is_hung_prose() {
        ...
        assert!(
            ...
            "flush After. / Next. must stay a separate Prose after parsed-literal, got {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
    ...
    fn leftover_meta_field_marker_is_structure_body_is_hung_prose() {
        ...
        assert!(
            ...
            "same-line meta field body must be leftover Prose, got {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
    ...
    /// header / footer same-line body hangs and splits; flush After. /
    /// Next. stay unindented. epigraph / replace:: stay unchanged.
    ...
    #[test]
    fn leftover_header_same_line_body_hangs_and_splits() {
        use crate::format::Format;
        use crate::{FormatConfig, format_text};

        let input = leftover_header_fixture();
        let regions = RstParser.parse(input);
        assert!(
            regions
                .iter()
        ...
        assert!(
            ...
            "flush After. / Next. must stay unindented Prose, got {regions:?}"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "header same-line body must hang and split; After. / Next. stay flush, got:\n{out}"
        );
        assert!(
    ...
    #[test]
    fn leftover_footer_same_line_body_hangs_and_splits() {
        use crate::format::Format;
        use crate::{FormatConfig, format_text};

        let input = leftover_footer_fixture();
        let regions = RstParser.parse(input);
        assert!(
            regions
                .iter()
        ...
        assert_eq!(
            ...
            "footer same-line body must hang and split; After. / Next. stay flush, got:\n{out}"
        );
        assert!(
    ...
    #[test]
    fn leftover_epigraph_and_replace_stay_unchanged() {
        use crate::format::Format;
        use crate::{FormatConfig, format_text};
        ...
        let epigraph_out = format_text(epigraph, &cfg).unwrap();
        assert_eq!(
            epigraph_out,
            concat!(
                ".. epigraph::\n",
        ...
        let replace_out = format_text(replace, &cfg).unwrap();
        assert_eq!(
            replace_out,
            concat!(
                "See |v|.\n",
