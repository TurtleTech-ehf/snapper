...
fn parse_line_based(input: &str) -> Vec<SpannedRegion> {
    ...
    while i < total {
        ...
        // Inside comment body (indented lines after a bare `..` or `.. text`).
        if in_comment {
            let leading = line_text.len() - line_text.trim_start().len();
        ...
        // RST comment: bare `..` or `.. text` without `::`. Following
        // indented lines stay Structure so splice does not outdent them.
...
/// True when `trimmed` is an RST comment opener: a bare `..`, or `..`
/// plus space/tab, and not a directive (`::`).
...
mod tests {
        ...
        assert!(
            !regions.iter().any(|r| {
                matches!(
                    r,
                    Region::Prose(s)
                        if s.contains("First") || s.contains("Second") || s.contains("..")
                )
            }),
            "comment must not be Prose, got {regions:?}"
        );
    }
    ...
    fn reporter_comment_body_is_identity_under_format() {
        ...
        assert_eq!(
            out, input,
            "bare .. then two indented sentences must stay identity under format, got:\n{out}"
        );
        assert!(
            out.starts_with("..\n"),
            "comment opener must stay bare .., got:\n{out}"
        );
        assert!(
            out.contains("\n   First sentence."),
            "first body line must keep indent, got:\n{out}"
        );
        assert!(
            out.contains("\n   Second sentence."),
            "second body line must keep indent, got:\n{out}"
        );
    }

    #[test]
    fn recognized_comment_body_stays_structure() {
        let input = ".. First sentence.\n   Second sentence.\n";
        let regions = RstParser.parse(input);
        assert!(
            regions
                .iter()
                .any(|r| matches!(r, Region::Structure(s) if s.contains("First sentence."))),
            "comment opener line must be Structure, got {regions:?}"
        );
        assert!(
            regions
                .iter()
                .any(|r| matches!(r, Region::Structure(s) if s.contains("Second sentence."))),
            "indented comment body must be Structure, got {regions:?}"
        );
        assert!(
            !regions
                .iter()
    ...
    #[test]
    fn recognized_comment_body_is_identity_under_format() {
        use crate::format::Format;
        use crate::{FormatConfig, format_text};
        ...
        assert_eq!(
            out, input,
            "recognized .. comment body must stay identity under format, got:\n{out}"
        );
        assert!(
            out.contains("\n   Second sentence."),
            "recognized comment body must keep indent, got:\n{out}"
        );
    }
}
