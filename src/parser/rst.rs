...
fn parse_line_based(input: &str) -> Vec<SpannedRegion> {
    ...
    // Compact `| ` line-block + flush paragraph (GitHub #409): the body
    // hangs at the `| ` width, so a column-0 line that is not `| ` must
    // close that hang instead of joining After. into the line-block.
    ...
    while i < total {
        ...
        // Prose so SemBr still splits (GitHub #174). A flush line that
        // is not `| ` ends the block (GitHub #409).
        ...
        if let Some(hang) = list_hang {
            ...
            list_hang = None;
            // Docutils ends a container or line-block at the first
            // less-indented line. Without a blank, the hung body is
            // still in current_prose; flush so After markup stays
            // column-0 Prose (GitHub #344 / #409).
...
mod tests {
    ...
    /// line that is not `| ` and not a hang. Following flush prose is a
    /// new paragraph.
    fn leftover_line_block_flush_fixture() -> &'static str {
        concat!("| First line. Second line.\n", "After. Next.\n")
    }

    #[test]
    fn leftover_line_block_does_not_swallow_flush_prose() {
        use crate::format::Format;
        use crate::{FormatConfig, format_text};

        let input = leftover_line_block_flush_fixture();
        let regions = RstParser.parse(input);
        assert!(
            regions
                .iter()
        ...
        assert!(
            !regions.iter().any(|r| matches!(
                r,
                Region::Structure(s)
                    if s.contains("First line")
                        || s.contains("After.")
                        || s.contains("Next.")
            )),
            "line-block and following sentences must not be Structure, got {regions:?}"
        );
        ...
        assert_eq!(
            out,
            concat!("| First line.\n", "  Second line.\n", "After.\n", "Next.\n",),
            "line-block hangs and splits; After. / Next. stay flush, got:\n{out}"
        );
        assert!(
            !out.contains("\n  After."),
