...
impl OrgParser {
    ...
    /// Planning must stay the line immediately after the headline; treating
    /// these as Prose joins them with the next paragraph and drops them.
...
impl FormatParser for OrgParser {
    fn parse_full(&self, input: &str) -> Vec<SpannedRegion> {
        ...
        for line in iter_lines(input) {
            ...
            // Planning / clock under a headline (org-element). Whole line
            // is Structure so DEADLINE:/CLOCK: do not join the next paragraph.
...
mod tests {
    ...
    /// Ticket fixture (Format::Org): planning after the headline and a
    /// CLOCK line later in the section (snapper-fcvd / GitHub #111).
    ...
    #[test]
    fn deadline_after_headline_is_structure() {
        use crate::format_text;

        // Isolated so origin/main cannot pass by joining into one paragraph.
        let input = "* TODO Task\nDEADLINE: <2026-01-01 Wed>\nBody starts here. Second sentence.\n";
        let regions = OrgParser.parse(input);
        assert!(
            regions.iter().any(|r| matches!(
            ...
            "DEADLINE: after a headline must be Structure, got: {regions:?}"
        );
        assert!(
            !regions
                .iter()
        ...
        assert!(
            ...
            "body after planning must stay Prose, got: {regions:?}"
        );
        let out = format_text(input, &org_cfg()).unwrap();
        assert!(
            out.contains("* TODO Task\nDEADLINE: <2026-01-01 Wed>\n"),
            "planning must stay the line immediately after the headline, got:\n{out}"
        );
        assert!(
            !out.contains("DEADLINE: <2026-01-01 Wed> Body starts here."),
            "DEADLINE: must not join the following paragraph, got:\n{out}"
        );
        assert!(
    ...
    #[test]
    fn scheduled_after_headline_is_structure() {
        use crate::format_text;
        ...
        assert!(
            ...
            "SCHEDULED: after a headline must be Structure, got: {regions:?}"
        );
        assert!(
            !regions
                .iter()
                .any(|r| matches!(r, Region::Prose(p) if p.contains("SCHEDULED:"))),
            "SCHEDULED: must not be Prose, got: {regions:?}"
        );
        let out = format_text(input, &org_cfg()).unwrap();
        assert!(
            out.contains("* TODO Task\nSCHEDULED: <2026-01-02 Thu>\n"),
            "SCHEDULED: must stay immediately after the headline, got:\n{out}"
        );
        assert!(
            !out.contains("SCHEDULED: <2026-01-02 Thu> Body starts here."),
            "SCHEDULED: must not join the following paragraph, got:\n{out}"
        );
        assert_eq!(format_text(&out, &org_cfg()).unwrap(), out);
    }

    #[test]
    fn closed_after_headline_is_structure() {
        use crate::format_text;
        ...
        assert!(
            ...
            "CLOSED: after a headline must be Structure, got: {regions:?}"
        );
        assert!(
            !regions
                .iter()
                .any(|r| matches!(r, Region::Prose(p) if p.contains("CLOSED:"))),
            "CLOSED: must not be Prose, got: {regions:?}"
        );
        let out = format_text(input, &org_cfg()).unwrap();
        assert!(
            out.contains("* DONE Task\nCLOSED: [2026-01-01 Wed 09:00]\n"),
            "CLOSED: must stay immediately after the headline, got:\n{out}"
        );
        assert!(
            !out.contains("CLOSED: [2026-01-01 Wed 09:00] Body starts here."),
            "CLOSED: must not join the following paragraph, got:\n{out}"
        );
        assert_eq!(format_text(&out, &org_cfg()).unwrap(), out);
    }

    #[test]
    fn clock_line_is_structure() {
        use crate::format_text;

        // Isolated from a headline so origin/main cannot pass via drawers.
        let input = "CLOCK: [2026-01-01 Thu 10:00]--[2026-01-01 Thu 11:00] =>  1:00\nNotes after clock. More notes.\n";
        let regions = OrgParser.parse(input);
        assert!(
            regions.iter().any(|r| matches!(
                r,
                Region::Structure(s)
                    if s.contains("CLOCK:")
                        && s.contains("[2026-01-01 Thu 10:00]--[2026-01-01 Thu 11:00] =>  1:00")
            )),
        ...
        assert!(
            ...
            "CLOCK: must not join the following paragraph, got:\n{out}"
        );
        assert!(
    ...
    #[test]
    fn fcvd_fixture_planning_and_clock_stay_structure() {
        use crate::format_text;
        ...
        assert!(
            ...
            "ticket DEADLINE: must be Structure, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
            ...
            "ticket CLOCK: must be Structure, got: {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
        ...
        assert!(
            out.contains("* TODO Task\nDEADLINE: <2026-01-01 Wed>\nBody starts here."),
            "planning must remain immediately after the headline, got:\n{out}"
        );
        assert!(
            !out.contains("DEADLINE: <2026-01-01 Wed> Body starts here."),
            "DEADLINE: must not join the following paragraph, got:\n{out}"
        ...
        assert!(
            out.contains("Body starts here.\nSecond sentence."),
            "body after planning must still reflow, got:\n{out}"
        );
        assert!(
            out.contains("Notes after clock.\nMore notes."),
            "notes after CLOCK: must still reflow, got:\n{out}"
        );
        assert_eq!(format_text(&out, &org_cfg()).unwrap(), out);
    }
}
