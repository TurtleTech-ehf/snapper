...
impl OrgParser {
    ...
    /// org-element quote/verse/center and special-block contain paragraphs.
    /// Lesser blocks (src, example, export, comment) stay literal.
...
impl FormatParser for OrgParser {
    fn parse_full(&self, input: &str) -> Vec<SpannedRegion> {
        ...
        for line in iter_lines(input) {
            ...
            // Inside example/export/comment (lesser blocks): the body is
            // opaque structure. Nested BEGIN/END still match by NAME (zyjn);
            // do not parse inner special-block / quote prose here.
            ...
            // #+BEGIN_NAME: fence is structure. quote/verse/center/special-block
            // parse inner content; lesser names stay opaque via in_literal_block.
