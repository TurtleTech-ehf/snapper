...
mod tests {
    ...
    /// src stays one token so an interior period is not a sentence
    /// boundary. `Next sentence.` still splits.
    ...
    #[test]
    fn org_inline_babel_call_interior_punct_is_not_a_sentence_boundary() {
        use crate::format_text;
        ...
        assert!(
            ...
            "sentence after the babel call must reflow, got:\n{out}"
        );
        assert!(
            !out.contains("1.\n2") && !out.contains("call_name(1.\n"),
            "must not split inside the babel call, got:\n{out}"
        );
        assert_eq!(format_text(&out, &org_cfg()).unwrap(), out);
    }
