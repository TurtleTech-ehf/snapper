...
pub fn dropped_comment_kind(input: &str, format: &str) -> Option<&'static str> {
    ...
    if protect::is_rst_pandoc_format(format)
...
///
/// RST `..` comments and `snapper:off` / `snapper:on` are lifted out
/// before the reader (tokens → `RawBlock`), then restored so write-through
/// exits 0 with that text still present.
pub fn format_via_pandoc(
    ...
    let protected = protect::protect_dropped_text(input, format);
    let json = json_with_backend(&protected.source, format, backend)?;
    let mut doc: pandoc_ast::Pandoc =
        serde_json::from_str(&json).map_err(|e| PandocError::Ast(e.to_string()))?;
    protect::inject_raw_blocks(&mut doc, &protected.parts, format);
    ...
    let restored = protect::restore_dropped_text(&written, &protected.parts);
    refuse_if_comments_missing(input, format, &restored)?;
    Ok(restored)
}
...
mod tests {
    ...
    #[test]
    fn format_via_pandoc_rst_comment_is_kept() {
        ...
        assert!(
            out.contains(".."),
            "RST comment opener must survive:\n{out}"
        );
    }
    ...
    #[test]
    fn format_via_pandoc_pragma_is_kept() {
        if !pandoc_available() && !ffi_write_available() {
            return;
        }
