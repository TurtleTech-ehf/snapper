...
/// so [`SENTENCE_CLOSERS`] can peel it. Leave other trailing `)` alone.
...
fn md_title_then_close(s: &str) -> bool {
    ...
    while i < bytes.len() {
        match bytes[i] {
            b'\\' if i + 1 < bytes.len() => i += 2,
            b if b == close => {
                return s[i + 1..].trim_start_matches([' ', '\t']) == ")";
            }
            b'\n' | b'\r' => return false,
            _ => i += 1,
        }
