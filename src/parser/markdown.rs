...
/// Rest of `line` after an optional CommonMark 0–3 space indent.
/// A tab or a fourth space is not a blockquote marker (CM 5.1).
fn after_quote_indent(line: &str) -> &str {
    let bytes = line.as_bytes();
    let mut i = 0;
    while i < bytes.len() && i < 3 && bytes[i] == b' ' {
        i += 1;
...
/// Each marker is `>` plus an optional space. Only 0–3 leading spaces
/// are skipped; four spaces or a tab is not a `>` prefix (snapper-cydz).
fn quote_marker_depth(line: &str) -> usize {
    let mut rest = after_quote_indent(line);
    let mut depth = 0;
...
/// Leading indent matches [`quote_marker_depth`] (0–3 spaces, not a tab).
...
/// same depth — is not an opener. A deeper quote prefix (`>` then `>>`,
/// or `> Foo` then `> > Bar`) is a nested opener (snapper-0772).
fn is_interrupting_setext_opener(line: &str, in_quote: bool, open_quote_depth: usize) -> bool {
    let depth = quote_marker_depth(line);
    if depth > 0 && !in_quote {
        return true;
    }
    if in_quote && depth > open_quote_depth {
        return true;
    }
    is_list_opener_line(line)
}
...
/// paragraph (ex. 93), not a heading. A shallower `>` is the same lazy
...
/// the item. Column-0 is relative to the list container after that
/// strip: a quoted opener (`> - Foo`) plus hung `>   Bar` then
/// `> =======` is the same reject as unquoted `- Foo` / `  Bar` /
/// `=======` (snapper-705k / snapper-u5ku). `heading_is_list` is true
/// when the last title line *or* an earlier open-paragraph line is a
/// list opener, including after quote markers. A quoted list marker
/// plus a quoted hung underline is still a heading: strip the list
/// layer after the matching quote depth.
fn is_setext_pair(
    title_line: &str,
    underline: &str,
    ...
    // Leftover `>` after the heading depth is a deeper nested quote.
    if quote_marker_depth(under_body) > 0 {
        return false;
    }
    ...
    // outside the list item. Do not require the last title line itself
    // to be the opener.
...
/// Quote depth of the heading container for an underline check.
/// An interrupting last title line (nested `>` or a list opener) is
/// its own container; otherwise the open paragraph's depth.
fn heading_container_quote_depth(
    last_line: &str,
    in_quote: bool,
    open_quote_depth: usize,
    interrupting: bool,
) -> usize {
    if interrupting {
        quote_marker_depth(last_line)
    } else if in_quote {
        open_quote_depth
    } else {
        quote_marker_depth(last_line)
...
impl FormatParser for MarkdownParser {
    fn parse_full(&self, input: &str) -> Vec<SpannedRegion> {
        ...
        while i < total {
            ...
            // Interrupting last line is its own heading; otherwise the
            // open paragraph. A list opener anywhere in that span plus
            // a 0-indent under_body is outside the item (snapper-705k).
            let start = if interrupting { i } else { para_start };
            let heading_is_list = (start..=i).any(|j| is_list_opener_line(lines[j].text));
            if i + 1 < total
                && is_setext_pair(
                    line_text,
                    lines[i + 1].text,
                ...
                )
            {
                ...
                // Column-0 is relative to the list container after
                // stripping heading_quote_depth. Do not skip QUOTE_RE
                // start lines (`> - Foo` / `>   Bar` / `> =======`).
                ...
                let quote_lazy_under = heading_quote_depth > 0
                    && quote_marker_depth(lines[i + 1].text) != heading_quote_depth;
            ...
            if let Some(caps) = QUOTE_RE.captures(line_text) {
                ...
                // a later underline can promote every title line. A nested
                // `>>` or a list opener (`> -`) must close the outer
                // paragraph when first seen (snapper-qvjl); otherwise a
                // later matching-depth setext would promote Foo.
                let prev_quote = i > 0 && QUOTE_RE.is_match(lines[i - 1].text);
                let quote_interrupt = quote_marker_depth(line_text)
                    > if i > 0 {
                        quote_marker_depth(lines[i - 1].text)
                    } else {
                        0
                    }
                    || is_list_opener_line(line_text);
                let quote_para_cont = in_list_item && prev_quote && !quote_interrupt;
                if quote_para_cont {
                ...
                if text.trim().is_empty() {
                    // CommonMark 5.1 / snapper-3y8u: `>` / `> ` / `>   `
                    // is a blank inside the blockquote (ex. 237) and
                    // closes the paragraph. Keep the quote item open.
                    end_paragraph(
                        &mut current_prose,
                        &mut prose_span,
                        &mut para_span,
...
mod tests {
    ...
    /// snapper-0772: a nested `>>` opener interrupts the outer quote.
    #[test]
    fn nested_quote_opener_setext_does_not_promote_outer_quote() {
        ...
        assert!(
            ...
            "outer quote must stay Prose, got: {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
        ...
        assert!(
            ...
            "inner quote title must be Structure, got: {regions:?}"
        );
        let out = crate::format_text(input, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
        ...
        assert!(
            !out.contains("first title line.\nStill title.\n>> Bar"),
            "must not keep the outer quote fused into the inner heading, got:\n{out}"
        );
        assert!(
            out.contains("Body after setext.\nSecond body."),
            "body Prose must still split, got:\n{out}"
    ...
    /// snapper-0772: `> >` is the same nested opener as `>>`.
    #[test]
    fn spaced_nested_quote_opener_setext_does_not_promote_outer_quote() {
        let input = concat!(
            "> Foo is the first title line. Still title.\n",
            "> > Bar is the second title line.\n",
            "> > =======\n",
    ...
    /// snapper-6g55: a column-0 list after a quote is a heading, not quote prose.
    #[test]
    fn list_after_quote_is_setext_heading() {
        let input = concat!(
        ...
        assert!(
            ...
            "list title must not sentence-split, got:\n{out}"
        );
        assert!(
            out.contains("Body after setext.\nSecond body."),
            "body Prose must still split, got:\n{out}"
    ...
    /// snapper-6g55: a quoted list marker plus hung underline is the heading.
    #[test]
    fn quoted_list_after_quote_is_setext_heading() {
        let input = concat!(
            "> Foo is quoted. Still quoted.\n",
            "> - Bar is the title\n",
            ">   =======\n",
    ...
    /// snapper-3y8u: `>` / `> ` / `>   ` closes the quote paragraph
    /// (CommonMark 5.1 ex. 237). Later same-depth setext is a new heading.
    #[test]
    fn empty_quote_line_closes_paragraph_before_later_setext() {
        ...
        assert!(
            ...
            "empty quote line must leave Foo as Prose, got: {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
        ...
        assert!(
            ...
            ">   blank must leave Foo as Prose, got: {regions:?}"
        );
    }
    ...
    fn deeper_quoted_underline_is_not_setext() {
        ...
        let regions = MarkdownParser.parse(one_line);
        assert!(
            regions.iter().any(|r| matches!(
                r,
                Region::Prose(p)
            ...
            "deeper >> ======= must leave Foo as Prose, got: {regions:?}"
        );
        let out = crate::format_text(one_line, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
            "Foo must still split under a deeper underline, got:\n{out}"
        );
        assert!(
            out.contains("Body after setext.\nSecond body."),
            "body Prose must still split, got:\n{out}"
        ...
        assert!(
            ...
            "multi-line title plus >> ======= must stay Prose, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
        ...
        assert!(
            ...
            "> > ======= is the same deeper opener, got: {regions:?}"
        );
    }

    /// snapper-p70e / snapper-l3rg: a shallower underline is lazy (ex. 93
    /// at nested depth), not a heading.
    #[test]
    fn shallower_quoted_underline_is_not_setext() {
        ...
        assert!(
            ...
            "shallower > ======= must leave Foo as Prose, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
            ...
            "Bar must stay Prose when the underline is shallower, got: {regions:?}"
        );
        let out = crate::format_text(multi, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
            "Foo must still split under a shallower underline, got:\n{out}"
        );
        assert!(
            out.contains("Body after setext.\nSecond body."),
            "body Prose must still split, got:\n{out}"
        ...
        assert!(
            ...
            "one-line >> Foo plus > ======= must stay Prose, got: {regions:?}"
        );
        ...
        assert!(
            ...
            ">>> Foo plus shallower >> ======= must stay Prose, got: {regions:?}"
        );
    }

    /// After a 0772 interrupt, a shallower underline is still lazy for Bar.
    #[test]
    fn interrupting_nested_title_then_shallower_underline_is_not_setext() {
        let input = concat!(
            "> Foo is the first title line. Still title.\n",
            ">> Bar is the second title line.\n",
            "> =======\n",
        ...
        assert!(
            ...
            "Bar plus shallower > ======= must stay inner quote Prose, got: {regions:?}"
        );
        let out = crate::format_text(input, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
    ...
    /// snapper-705k / snapper-u5ku: after quote strip, column-0 `=======`
    /// is outside a quoted list item. Hung `>   =======` stays a heading.
    #[test]
    fn quoted_multiline_list_column0_underline_is_not_setext() {
        ...
        assert!(
            ...
            "hung list continuation must stay Prose, got: {regions:?}"
        );
        assert!(
            !regions.iter().any(|r| matches!(
                r,
            ...
            "column-0 quote underline must not promote Foo, got: {regions:?}"
        );
        let out = crate::format_text(input, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
        ...
        let regions = MarkdownParser.parse(hung);
        assert!(
            !regions.iter().any(|r| matches!(
                r,
                Region::Prose(p)
            ...
            "hung >   ======= must stay a list setext, got: {regions:?}"
        );
    }

    /// snapper-cydz: 4 spaces or a tab before `>` is not a quote marker.
    #[test]
    fn four_space_or_tab_quoted_underline_is_not_setext() {
        ...
        assert!(
            ...
            "4-space > ======= must leave Foo as Prose, got: {regions:?}"
        );
        let out = crate::format_text(four, &md_cfg()).unwrap();
        assert!(
            out.contains("first title line.\n"),
            "Foo must still split under a 4-space underline, got:\n{out}"
        );
        assert!(
            out.contains("Body after setext.\nSecond body."),
            "body Prose must still split, got:\n{out}"
        ...
        assert!(
            ...
            "multi-line title plus 4-space > ======= must stay Prose, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
            ...
            "Bar must stay Prose when the underline is 4-space quoted, got: {regions:?}"
        );
        ...
        let regions = MarkdownParser.parse(tab);
        assert!(
            regions.iter().any(|r| matches!(
                r,
                Region::Prose(p)
            ...
            "tab-prefixed > ======= must leave Foo as Prose, got: {regions:?}"
        );
        ...
        let regions = MarkdownParser.parse(three);
        assert!(
            !regions.iter().any(|r| matches!(
                r,
                Region::Prose(p)
            ...
            "3-space > ======= is a real closer, got: {regions:?}"
        );
    }

    /// snapper-qvjl: nested `>>` mid-title closes the outer paragraph.
    #[test]
    fn nested_quote_multiline_inner_setext_leaves_outer_prose() {
        let input = concat!(
            "> Foo is the first title line. Still title.\n",
            ">> Bar is the second title line.\n",
        ...
        assert!(
            ...
            "inner Bar+Baz must be the setext, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
    ...
    #[test]
    fn quoted_list_multiline_after_quote_is_inner_setext() {
        let input = concat!(
            "> Foo is quoted. Still quoted.\n",
        ...
        assert!(
            ...
            "Bar+Baz must be the list setext, got: {regions:?}"
        );
        assert!(
            regions.iter().any(|r| matches!(
                r,
