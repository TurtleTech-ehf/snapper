...
/// `1. ` is 3). A setext underline indented `1..hang-1` is outside the
/// item (GitHub #260).
fn list_opener_hang(line: &str) -> Option<usize> {
    LIST_ITEM_RE
        .captures(quote_body(line).unwrap_or(line))
...
/// including after quote markers) plus an `under_body` indented below
/// the item hang is outside the item (GitHub #260). Column-0 is the
/// same reject (snapper-705k / snapper-u5ku). Do not require the last
/// title line itself to be the opener. A quoted list marker plus a
/// quoted hung underline (indent == hang) is still a heading.
fn is_setext_pair(
    title_line: &str,
    underline: &str,
    heading_quote_depth: usize,
    heading_list_hang: Option<usize>,
) -> bool {
    ...
    // CommonMark 5.2: a list item continues only at hang. Indent
    // 1..hang-1 (and column 0) is outside the item, so not a closer.
    if heading_list_hang.is_some_and(|hang| line_indent(under_body) < hang) {
        return false;
    }
    true
}
    ...
    fn parse_full(&self, input: &str) -> Vec<SpannedRegion> {
        ...
        while i < total {
                ...
                )
            {
                ...
                // Hang is relative to the list after stripping the
                // heading quote depth. Indent below hang is outside the
                // item (GitHub #260). Do not skip QUOTE_RE start lines
...
mod tests {
    ...
    /// snapper-grff / GitHub #260: underline indent 1..hang-1 is outside
    /// the list item. Hang of `- ` is 2, so one space is not a closer.
    #[test]
    fn list_underline_below_hang_is_not_setext() {
        let input = concat!(
            "- Foo is the first title line. Still title.\n",
            "  Bar is the second title line.\n",
            " =======\n",
        ...
        let quoted_out = crate::format_text(quoted, &md_cfg()).unwrap();
        assert!(
