...
"
    ...
    #[test]
    fn rst_alpha_roman_paren_enumerator_hanging_indent() {
        let result = reflow_regions(vec![
            Region::Structure("a. ".to_string()),
            Region::Prose("Alpha item. Second sentence.".to_string()),
            Region::Structure("\n".to_string()),
            Region::Structure("(1) ".to_string()),
            Region::Prose("Paren arabic. Second sentence.".to_string()),
            Region::Structure("\n".to_string()),
            Region::Structure("i. ".to_string()),
            Region::Prose("Roman item. Second sentence.".to_string()),
            Region::Structure("\n".to_string()),
        ]);
        assert_eq!(
            result,
    ...
    #[test]
    fn max_width_keeps_org_src_after_word_underscore_atomic() {
        // GitHub #408: org-element `\<src_` matches after a word.
        let token = "src_python{print(1. 2)}";
        ...
        for clause in [false, true] {
            ...
            assert!(
                !wrapped.contains("1.\n2"),
                "must not wrap on the interior period after word-underscore, got:\n{wrapped}"
            );
        }
    }
    ...
    #[test]
    fn max_width_keeps_org_call_after_word_underscore_atomic() {
        let token = "call_name(1. 2)";
